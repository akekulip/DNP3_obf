...
#ifdef SR4_READINESS
                        /* SR4: STRUCTURAL READINESS. Hold the OPERATE only when the qid3
                         * reservoir is confirmed resident for THIS generation; otherwise FAIL
                         * OPEN WITHOUT HOLDING (forward immediately). arm_clone() still seeds
                         * the reservoirs on both arms so the ACK/echo T0-anchored holds and a
                         * later shaped retry remain possible; a raced qid2 early-release can
                         * never happen because the OPERATE is never enqueued when unready. */
                        if (meta.op_ready == 8w1) {
                            TO_OP_HOLD()             /* proven resident: hold byte-identically, qid2 */
                            arm_clone();
                            ctr_fresh.count(CF_OP_HOLD);
                        } else {
                            D3_TO_FWD()              /* residency NOT proven: immediate fail-open forward */
                            arm_clone();
