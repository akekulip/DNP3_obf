...
control Ingress(inout headers_t hdr,
    ...
    apply {
        if (meta.port_ok == 8w0) {
            ...
            /* ================= ACT (UNIFIED12: leaves set meta.outcome ONLY) =========
             * Every leaf sets meta.outcome (plus any PRE-COMMIT header mutation and the
             * ts-event flag). The single tbl_commit — applied once after the telemetry —
             * is the ONLY writer of port/qid/bypass/drop/mcast and the ONLY counter site.
             * The predicate tree is byte-for-byte the frozen oracle's; only the terminal
             * statements changed (the to_ / D3_ / RRC_ / ctr.count leaves -> one meta.outcome). */
            if (meta.dequeued == 8w0) {
                /* ----- FRESH from a host port (or the pktgen source dp68) ----- */
                if (meta.role == ROLE_CLONE) {
                    meta.outcome = OUT_CLONE;                 /* drop */

                } else if (meta.role == ROLE_BLOCK) {
                    if (meta.is_pktgen == 8w1) {
                        /* PKTGEN admission: stamp gen+budget, then enqueue the reservoir. */
                        if (meta.txn_active == 8w1 && hdr.pgen.packet_id[15:7] != 9w0) {
                            meta.outcome = OUT_PKTGEN_DROP;   /* invalid id >= 128 */
                            ...
                            meta.outcome = OUT_ADMIT_ACK;     /* qid7 ACK reservoir    */
                            ...
                            meta.outcome = OUT_ADMIT_RESP;    /* qid5 RESP reservoir   */
                            meta.ev_first_block = 8w1;
                        } else {
                            meta.outcome = OUT_PKTGEN_DROP;   /* no active txn */
                        }
                    } else {
                        /* R3: a fresh non-pktgen 0x88C1 frame is a host injection -> reject
                         * before Q_BLOCK (distinct from CF_BLOCK_ENQ; the token never queued). */
                        meta.outcome = OUT_BLOCK_REJECT;
                    }

                } else if (meta.pkt_class == CLASS_RESP) {
                    /* Every in-transaction RESPONSE holds (qid4) UNCONDITIONALLY; rel_diff
                     * only ATTRIBUTES early/late. Duplicate suppressed; stale/no-txn bypass. */
                    if (meta.verdict == V_RESP && meta.txn_active == 8w1 &&
                        (meta.mode == MODE_OFF || meta.mode == MODE_FAIL_OPEN)) {
                        meta.outcome = OUT_RESP_OFF_FWD;      /* unreachable under OFF; loud alarm */
                    } else if (meta.verdict == V_RESP && meta.txn_active == 8w1) {
                        if (meta.tag_diff == 8w0) { meta.outcome = OUT_RESP_HOLD_LATE; }
                        else                      { meta.outcome = OUT_RESP_HOLD_EARLY; }
                    } else if (meta.verdict == V_RESP && meta.txn_active == 8w2) {
                        meta.outcome = OUT_RESP_DUP_SUPP;     /* drop: exact retransmit while held */
                    } else {
                        /* stale/wrong/no-txn: forward unprotected, carve iff shape-armed+49B. */
                        if (meta.do_shape == 16w1) { meta.outcome = OUT_RESP_BYPASS_SHAPE; }
                        else                       { meta.outcome = OUT_RESP_BYPASS_FWD; }
                    }

                } else if (meta.pkt_class == CLASS_ACK) {
                    if (meta.verdict == V_ACK_ARM &&
                        (meta.mode == MODE_OFF || meta.mode == MODE_FAIL_OPEN)) {
                        meta.outcome = OUT_ACK_REJECT;        /* OFF: forward, no hold */
                    } else if (meta.verdict == V_ACK_ARM) {
                        if (meta.dl_pre == UNARMED_WORD) {
                            meta.outcome = OUT_ACK_HOLD;      /* FIRST qualifying ACK: armed T_A */
                            meta.ev_ack_arm = 8w1;
                        } else {
                            meta.outcome = OUT_ACK_DUP_HOLD;  /* SECOND: held, no re-arm */
                        }
                    } else {
                        meta.outcome = OUT_ACK_REJECT;        /* failed a §8.1 conjunct */
                    }

                } else if (meta.pkt_class == CLASS_ARM) {
                    /* a real DNP3 READ: forwarded byte-identically. FRESH arms the clone. */
                    if (meta.verdict == V_ARM_FRESH &&
                        meta.mode != MODE_OFF && meta.mode != MODE_FAIL_OPEN) {
                        meta.outcome = OUT_ARM_FRESH;         /* fwd + one K=64 burst (clone) */
                    } else if (meta.verdict == V_ARM_DUP) {
                        meta.outcome = OUT_ARM_DUP;           /* retransmit: no burst */
                    } else {
                        meta.outcome = OUT_ARM_BUSY;          /* concurrent-txn escape */
                    }

                } else if (meta.role == ROLE_RESP_UNSUP) {
                    meta.outcome = OUT_UNSUP;                 /* fwd transparently */

                } else {
                    meta.outcome = OUT_BYPASS;                /* ROLE_BYPASS: transparent */
                }

            } else {
                /* ----- DEQUEUED (looped back from dp8) ----- */
                if (meta.role == ROLE_BLOCK && meta.is_resp_blk == 8w1) {
                    /* RESP blocker (qid5): stale > deadline(pending) > budget > loop. */
                    if (meta.verdict != V_BLOCK_LIVE && meta.verdict != V_BLOCK_PENDING) {
                        meta.outcome = OUT_RB_STALE;
                        meta.ev_block_term = 8w1;
                    } else if (meta.verdict == V_BLOCK_PENDING && meta.expired_resp == 8w1) {
                        meta.outcome = OUT_RB_DL;             /* T_RESP reached, RESP held: release */
                        meta.ev_block_term = 8w1;
                    } else if (meta.budget_zero == 8w1) {
                        meta.outcome = OUT_RB_TMO;            /* RESP_MISSING / fail-open horizon */
                        meta.ev_block_term = 8w1;
                    } else {
                        hdr.ib.seq = hdr.ib.seq - 32w1;
                        meta.outcome = OUT_RB_LOOP;           /* re-enqueue qid5 */
                    }

                } else if (meta.role == ROLE_BLOCK) {
                    /* ACK blocker (qid7): stale > [D1 event|budget] > deadline > budget > loop. */
                    if (meta.verdict != V_BLOCK_LIVE && meta.verdict != V_BLOCK_PENDING) {
                        meta.outcome = OUT_AB_STALE;
                        meta.ev_block_term = 8w1;
                    } else if (meta.mode == MODE_D1_EVENT) {
                        if (meta.verdict == V_BLOCK_PENDING) {
                            meta.outcome = OUT_AB_DL;         /* RESP observed -> release ACK */
                            meta.ev_block_term = 8w1;
                        } else if (meta.budget_zero == 8w1) {
                            meta.outcome = OUT_AB_TMO;        /* missing RESPONSE fail-open */
                            meta.ev_block_term = 8w1;
                        } else {
                            hdr.ib.seq = hdr.ib.seq - 32w1;
                            meta.outcome = OUT_AB_LOOP;       /* keep holding the ACK */
                        }
                    } else if (meta.expired == 8w1) {
                        meta.outcome = OUT_AB_DL;             /* D2/D3/D4: ACK deadline T_A */
                        meta.ev_block_term = 8w1;
                    } else if (meta.budget_zero == 8w1) {
                        meta.outcome = OUT_AB_TMO;            /* ACK_MISSING_FAIL_OPEN */
                        meta.ev_block_term = 8w1;
                    } else {
                        hdr.ib.seq = hdr.ib.seq - 32w1;
                        meta.outcome = OUT_AB_LOOP;
                    }

                } else if (meta.role == ROLE_ACK) {
                    /* ACK RELEASE pass: forward byte-identically (dp9 qid0). */
                    if ((meta.mode == MODE_D1_EVENT || meta.mode == MODE_D3_ACK) && meta.txn_active == 8w1) {
                        meta.outcome = OUT_ACK_REL_RETIRE;   /* D1/D3 retired on ACK release */
                    } else {
                        meta.outcome = OUT_ACK_RELEASE;      /* D2/D4 preserved, or RESP queued */
                    }

                } else if (meta.role == ROLE_RESP) {
                    /* RELEASED RESPONSE: the timing-release event invokes the RRC carve
                     * (shape iff armed+eligible), attributed by its OWN deadline (T_RESP). */
                    if (meta.expired_resp == 8w1) {
                        if (meta.do_shape == 16w1) { meta.outcome = OUT_REL_DL_SHAPE; }
                        else                       { meta.outcome = OUT_REL_DL_FWD; }
                    } else {
                        if (meta.do_shape == 16w1) { meta.outcome = OUT_REL_FO_SHAPE; }
                        else                       { meta.outcome = OUT_REL_FO_FWD; }
                    }

                } else {
                    meta.outcome = OUT_DEQ_DROP;              /* nothing else may loop back */
                }
            }
