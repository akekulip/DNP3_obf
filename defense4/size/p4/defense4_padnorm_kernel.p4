/* ============================================================================
 * defense4_padnorm_kernel.p4 — Defense 4 SIZE compile-probe: PAD-normalization of
 *   the MEASURED non-block-aligned SEL-751 responses (READ 40 B, SBO 49 B) to a
 *   common 64 B target by reconstructing the short trailing DNP3 block as
 *   [real-prefix ++ zero-fill] with a runtime block CRC. Tofino-1 / TNA.
 *   COMPILE PROBE (software/model only — NOT silicon-validated).
 *
 * Sibling of defense4_split_kernel.p4; INGRESS (Case-A timing core) + its parser/
 * deparser + all ingress constants/headers/metadata are copied VERBATIM from
 * defense4_cover_kernel.p4 (lines 46-130 and 212-638), byte-identical, NOT modified.
 * Only the EGRESS is new. This kernel isolates the block-alignment barrier from the
 * (unrelated, TF1-infeasible) multi-boundary transport wall — see the EGRESS banner.
 * ==========================================================================*/
#include <core.p4>
#include <tna.p4>
...
/* ---- egress size-layer headers (v2 PAD-normalization: handles NON-block-aligned
 *      real frames by reconstructing the short trailing block as [real ++ zeros].
 *      Egress-only. ---- */
header pay18_h { bit<144> b; }              /* full 18-byte DNP3 block (16 user + 2 CRC) */
header p1r_h   { bit<80>  user; bit<16> crc; }  /* READ block1: 10 user + 2 CRC (12 B) */
header p2s_h   { bit<8>   user; bit<16> crc; }  /* SBO  block2:  1 user + 2 CRC ( 3 B) */
header obmix_h { bit<128> user; bit<16> crc; }  /* reconstructed mixed block (18 B)     */
...
struct headers_t {
    ...
    dnp3_app_h  app;
    /* egress size layer */
    pay18_h     blk0;      /* full first data block (both response types)   */
    pay18_h     blk1;      /* full second data block (SBO only)             */
    p1r_h       p1r;       /* READ partial block1 (parsed, not emitted)     */
    p2s_h       p2s;       /* SBO  partial block2 (parsed, not emitted)     */
    obmix_h     ob_mix;    /* reconstructed mixed block (emitted)           */
    pay18_h     ob_fill;   /* constant filler block (READ only)             */
}
struct ig_meta_t {
    bit<8>  role;
    bit<8>  dir;
...
/* ======================================================================== *
 *  EGRESS — v2 PAD-NORMALIZATION (non-block-aligned): grow the MEASURED
 *  SEL-751 responses to a common target by reconstructing the short trailing
 *  DNP3 block as [real-prefix ++ zero-fill] and recomputing its CRC at runtime.
 *  Tofino-1 / TNA. COMPILE PROBE. PAD-only (no SPLIT).
 * ------------------------------------------------------------------------ *
 * Removes the block-alignment constraint of defense4_split_kernel.p4 so the real
 * on-wire sizes normalize to L=64 B (3 full 18-byte blocks, ip.total_len 104):
 *   READ  40 B (total_len 80, user 26 = block0(16) + 10 in block1) -> +24
 *   SBO   49 B (total_len 89, user 33 = block0(16)+block1(16)+ 1 in block2) -> +15
 * The trailing partial block is rebuilt as [K real user bytes ++ (16-K) zero bytes]
 * with its DNP3 CRC computed over the reconstructed 16 bytes by the native
 * CRCPolynomial hash (the joint_canon idiom). K is fixed per response type
 * (READ K=10, SBO K=1), so the hash field list is fixed. The header LEN is set to
 * 0x35 and its header-block CRC recomputed. Faithful to the reference model
 * build_frame(pad_user_to(real_user, 48)) — verified offline.
 *
 * ►► HONEST WALL (does NOT go away): this is a SINGLE-INSERTION epoch, so it
 *    normalizes at most ONE response per connection. A live stream of interleaved
 *    READ (+24) and SBO (+15) responses would need a per-response VARIABLE-delta
 *    multi-boundary transport ledger (reverse cumulative-ack inverse over unbounded
 *    boundaries), which the Tofino-1 SALU cannot maintain. So continuous stream
 *    size-normalization is NOT achievable on one TF1 chip; this kernel proves only
 *    that the block-alignment barrier is removable.
 *
 * FAIL-OPEN for any other size / non-owner / options / fragment. Reverse ACK is
 * translated by -clamp(ack-b0, 0, delta) for the one padded response.
 * ======================================================================== */

const bit<16> PORT_DNP3 = 16w20000;
const bit<8>  POL_NONE = 8w0;
const bit<8>  POL_PAD  = 8w1;
const bit<32> PDELTA_CLOSED = 32w0;

const bit<16> TLEN_READ = 16w80;    /* ip.total_len of the 40 B READ response  */
const bit<16> TLEN_SBO  = 16w89;    /* ip.total_len of the 49 B SBO  response  */
const bit<16> TLEN_TGT  = 16w104;   /* normalized: 3 full blocks (64 B on wire) */
const bit<8>  DL_LEN_TGT = 8w0x35;  /* LEN = 5 + 48 user bytes                  */
const bit<144> FILL_BLOCK = 144w0xFFFF;   /* 16 zero user bytes + CRC 0xFFFF     */
const bit<32> PAD_READ = 32w24;
const bit<32> PAD_SBO  = 32w15;

const bit<8> PATH_NONE = 8w0;
const bit<8> PATH_READ = 8w1;
const bit<8> PATH_SBO  = 8w2;

struct eg_meta_t {
    bit<10> flow_idx;
    bit<32> nm_ip; bit<32> nr_ip; bit<16> nm_pt; bit<16> nr_pt;
    bit<8>  dir;
    bit<8>  eligible; bit<8> has_opt; bit<8> is_ctl; bit<8> is_syn; bit<8> is_rst;
    bit<8>  owner; bit<8> policy; bit<8> path;
    bit<32> pad_add;
    bit<128> mix_user;        /* reconstructed mixed-block user: real ++ zeros */
    bit<32> pdelta_old; bit<32> pb0_old;
    bit<8>  is_fwd; bit<8> is_rev; bit<8> open_req; bit<8> did_open;
    bit<32> a_diff; bit<32> ack_sub;
    bit<1>  translated; bit<16> tcp_len;
}
...
parser EgParser(packet_in pkt, out headers_t hdr, out eg_meta_t m,
    ...
    state start {
        ...
        m.dir = DIR_MASTER; m.eligible = 8w0; m.has_opt = 8w0; m.is_ctl = 8w0;
        m.is_syn = 8w0; m.is_rst = 8w0; m.owner = 8w0; m.policy = POL_NONE; m.path = PATH_NONE;
        m.pad_add = 32w0; m.mix_user = 128w0;
        m.pdelta_old = 32w0; m.pb0_old = 32w0;
        m.is_fwd = 8w0; m.is_rev = 8w0; m.open_req = 8w0; m.did_open = 8w0;
        m.a_diff = 32w0; m.ack_sub = 32w0; m.translated = 1w0; m.tcp_len = 16w0;
    ...
    state parse_tcp {
        pkt.extract(hdr.tcp);
        transition select(hdr.tcp.dofs) { 4w5 : mark_elig; default : tcp_opt; }
    }
    state tcp_opt { m.has_opt = 8w1; transition accept; }
    state mark_elig { m.eligible = 8w1; transition parse_dl; }
    state parse_dl {
        pkt.extract(hdr.dl);
        transition select(hdr.dl.start) { DNP3_START : parse_blk0; default : accept; }
    }
    state parse_blk0 {
        pkt.extract(hdr.blk0);
        transition select(hdr.ipv4.total_len) {
            TLEN_READ : parse_p1r;      /* READ: one full block then a 10-user partial */
            TLEN_SBO  : parse_blk1;     /* SBO : two full blocks then a 1-user partial  */
            default   : accept;
        }
    }
    state parse_p1r  { pkt.extract(hdr.p1r); transition sz_read; }
    state parse_blk1 { pkt.extract(hdr.blk1); transition parse_p2s; }
    state parse_p2s  { pkt.extract(hdr.p2s); transition sz_sbo; }
    state sz_read { m.path = PATH_READ; transition accept; }
    state sz_sbo  { m.path = PATH_SBO;  transition accept; }
}
...
control Egress(inout headers_t hdr, inout eg_meta_t m,
    ...
    Hash<bit<16>>(HashAlgorithm_t.CUSTOM, dnp3_poly) h_mix;
    ...
    /* single-insertion epoch (reg_pdelta / reg_pb0), owner-gated */
    ...
    table t_adiff_cls {
        ...
        const entries = {
            (32w0 &&& 32w0xFFFFFFFF) : asub_zero();
            (32w0 &&& 32w0xFFFFFFE0) : asub_snap();   /* 1..31 (both deltas < 32) */
            (32w0 &&& 32w0x80000000) : asub_full();
        }
        size = 4;
    }
    ...
    apply {
        if (hdr.tcp.isValid()) {
            ...
            /* per-response-type delta + reconstructed mixed-block user (fixed layout per path) */
            if (m.path == PATH_READ) { m.pad_add = PAD_READ; m.mix_user = hdr.p1r.user ++ 48w0; }
            else if (m.path == PATH_SBO) { m.pad_add = PAD_SBO; m.mix_user = hdr.p2s.user ++ 120w0; }

            if (m.policy == POL_PAD && m.owner == 8w1 && m.eligible == 8w1
                && m.is_ctl == 8w0 && m.is_fwd == 8w1 && m.path != PATH_NONE) {
                m.open_req = 8w1;
            }

            /* epoch registers (owner + eligible) */
            ...
            /* ---- materialise the normalized frame on the padded opener ---- */
            if (m.did_open == 8w1) {
                hdr.dl.len = DL_LEN_TGT;
                hdr.ob_mix.setValid();
                hdr.ob_mix.user = m.mix_user;
                if (m.path == PATH_READ) {          /* block2 = full zero filler */
                    hdr.ob_fill.setValid();
                    hdr.ob_fill.b = FILL_BLOCK;
                }
                hdr.ipv4.total_len = TLEN_TGT;
                m.translated = 1w1;
            }
            /* reverse ACK translation for the one open epoch */
            if (m.is_rev == 8w1 && m.owner == 8w1 && m.eligible == 8w1
                && m.pdelta_old != PDELTA_CLOSED && m.ack_sub != 32w0) {
                hdr.tcp.ack = hdr.tcp.ack - m.ack_sub; m.translated = 1w1;
            }

            /* ---- runtime DNP3 CRCs on the padded opener ---- */
            if (m.did_open == 8w1) {
                bit<16> c_dl = h_dlcrc.get({ hdr.dl.start, hdr.dl.len, hdr.dl.ctrl, hdr.dl.dst, hdr.dl.src });
                hdr.dl.crc = c_dl[7:0] ++ c_dl[15:8];
                bit<16> c_mx = h_mix.get({ m.mix_user });
                hdr.ob_mix.crc = c_mx[7:0] ++ c_mx[15:8];
            }
...
/* ============================= egress deparser ==========================
 * Emit order dl, blk0, blk1, ob_mix, ob_fill. POV gating gives both layouts:
 *   READ: blk1 invalid, ob_mix valid (block1), ob_fill valid (block2)
 *   SBO : blk1 valid  (block1), ob_mix valid (block2), ob_fill invalid
 * The parsed partial blocks p1r/p2s are consumed (not emitted). */
    ...
    apply {
        if (m.translated == 1w1) {
            ...
            hdr.tcp.csum = tcp_csum.update({
                ...
                hdr.blk0.b, hdr.blk1.b,
                hdr.ob_mix.user, hdr.ob_mix.crc, hdr.ob_fill.b });
        }
        pkt.emit(hdr.eth);
        pkt.emit(hdr.ipv4);
        pkt.emit(hdr.tcp);
