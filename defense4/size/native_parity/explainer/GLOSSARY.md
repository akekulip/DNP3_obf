...
**RRC (Response Recirculate + Carve).** The **size + CLRT** primitive for READ/SELECT
responses: recirculate the response on **dp8**, **carve** it at a DNP3 CRC block boundary into a
fixed segmentation ([28, 21] bytes for the SEL's 49-byte response, byte-preserving, no CRC
recompute), and **hold** it so the master-visible CLRT is a fixed policy value instead of the
device's native one.

**BOR (Bounded OPERATE Release).** The **timing** primitive for the SBO OPERATE: admit exactly
...
(the *anti-subtraction* property).

**T0, J, A (D_A), R (D_R).** T0 = OPERATE arrival time. J = secret release delay (from a bounded
codebook). A = master-visible ACK offset. R = master-visible response/echo offset. For RRC,
the normalized **CLRT = D_R − D_A**.
