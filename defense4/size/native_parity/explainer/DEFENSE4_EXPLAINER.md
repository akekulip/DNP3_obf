...
subtitle: "RRC (size + CLRT) and BOR (bounded OPERATE release), validated against a physical SEL-751"
...
|---|---|---|
| (a) CLRT (network) | firmware response timing on READ/SELECT | **RRC** — normalizes CLRT to a fixed 4 ms and fixes segmentation |
| (b) Physical operation time | the relay's true OPERATE actuation latency | **BOR** — releases the OPERATE at a hidden T0+J and anchors ACK/echo to T0, so the actuation latency and the hold are both unobservable (anti-subtraction) |

This is the core framing: **RRC defeats the cross-layer (network-timing) fingerprint; BOR defeats the
physical operation-time fingerprint.** Both run in-network, on one switch, without changing a DNP3
byte.

**What Defense 4 protects — and what it does not claim.** It **replaces** one relay's size,
segmentation, and CLRT signature with a fixed in-network policy, and it releases control commands
on a hidden delay so the command timing cannot be read off the wire. It **does not** claim
...
![**FIG-0.** Defense 4 high-level packet path: DNP3 traffic is classified, a timing policy is applied, the packet is steered into the RRC or BOR queues, the response is split at a CRC boundary, and the master sees normalized traffic.](figs/FIG-0_overview_flow.png){width=98%}
...
**Why an arbitrary split would be unsafe.** Each DNP3 16-byte block ends in its own CRC. If you cut
*inside* a block, a receiver validating block CRCs sees a broken frame. RRC therefore cuts **only
on a completed block boundary** — for the 49-byte SEL frame, after the link header + block 0 (28
bytes), leaving block 1 (21 bytes): **[28, 21]**, with `join([28],[21]) == [49]` byte for byte and
every block CRC still valid. No DNP3 field is modified and no CRC is recomputed.
...
one program and adds size normalization. A packet comes in, the switch decides what kind of
transaction it is, applies a timing policy, steers it into the right hold queues, splits the
response at a CRC boundary, and lets the master see uniform traffic.
...
| **RRC** response handling | recirculate + carve + hold (size + CLRT) | dp8 ladder (§5) |
| **BOR** OPERATE handling | admit one, hold, release at T0+J | dp10 ladder (§6) |
| **Size normalization** | [49] → [28,21] at a CRC boundary | egress split / deparser (§9) |

The two deadlines D_A (ACK) and D_R (response) are what make CLRT a constant: since
$\mathrm{CLRT} = D_R - D_A$, holding *both* the ACK and the response to fixed offsets pins the gap
...
at a CRC boundary into a fixed shape, and releases it at a fixed time — so both the *size/segments*
and the *CLRT* the master sees are policy values, not the relay's own.
...
their own blocker reservoirs (qid7, qid5). The control plane sets D_A ≈ 20 ms for the ACK and D_R ≈
4 ms for the response *relative to the ACK*, so the two release times are set independently and
$\mathrm{CLRT} = D_R - D_A$ is exactly the configured constant.

**The RRC timing model.**

$$T_{\text{ACK}} = T_0 + D_A, \qquad T_{\text{response}} = T_0 + D_A + D_R, \qquad
\mathrm{CLRT} = D_R$$

(measured from the ACK), and on silicon this lands at **CLRT = 4.001 ms, std 0.02** for both READ
and SELECT — replacing native 1.27 ms / 2.11 ms with high variance.
...
![**FIG-10.** The RRC and BOR primitives as process flows: RRC recirculates → carves at a CRC boundary → holds to a fixed CLRT; BOR pre-seeds an epoch at SELECT → holds the OPERATE → releases once at T0+J, with ACK/echo anchored to T0.](figs/FIG-10_rrc_bor_primitive.png){width=98%}

---

# 6. BOR in detail

**Plain version.** BOR handles the one dangerous thing — a control command. It lets exactly one
matching OPERATE through, holds it inside the switch, and releases it once at a *secret* delay,
while making sure the master sees the ACK and echo at fixed times that reveal nothing about that
delay. And if the OPERATE is retransmitted, BOR does not fire twice. This is what defeats Formby's
**physical operation-time** fingerprint (§1): an observer who times the OPERATE→confirmation gap
learns the configured policy offsets A and R, never the relay's true actuation latency or the hidden
hold J.
...
5. **The OPERATE is released once at T0 + J** to the relay on dp64.
6. **ACK and echo deadlines stay anchored to the original T0.** The ACK is released to the master at
   ...
   ingress timestamp T0 — *not* to the later relay ACK.
7. **Retransmissions meet transaction state, not a second operation.** A retransmitted OPERATE
   after release reads the *spent-epoch* marker, is classified a duplicate, and is dropped.
...
# 9. Size splitting mechanism

**Plain version.** The switch turns one 49-byte response into two TCP segments of 28 and 21 bytes,
cut exactly on a DNP3 CRC boundary, with the sequence numbers lined up so the master reassembles the
...
0 with its CRC (18 bytes) + data block 1 with its CRC (21 bytes). The only safe interior cut is on a
completed block boundary. Cutting after the header + block 0 gives **28 bytes**; the remainder is
**21 bytes**:
...
![**FIG-11.** The 49-byte SEL-751 DNP3 response and the cut point. The frame is a 10-byte link header (with its CRC) followed by CRC-protected data blocks. The only safe cut is on a completed block boundary: after the link header + block 0 (28 bytes), leaving block 1 + the final block (21 bytes). Segment 1 = [28], segment 2 = [21] with its TCP sequence advanced by 28; no DNP3 byte is changed and no CRC is recomputed.](figs/FIG-11_dnp3_frame_cut.png){width=98%}
...
| # | Result | Value | Source |
|---|---|---|---|
| Size (native) | READ responses | **100 × [49]**, all CRC/checksum-valid | `size_verdict.csv` |
| Size (defended READ) | segments | **600 × [28,21]** | `size_verdict.csv` |
| Size (defended SELECT) | segments | **500 × [28,21]** | `size_verdict.csv` |
| Size (SBO) | class split | **90 SELECT + 90 OPERATE**, all [28,21] | `sbo_*.csv` |
| Size escapes | 49-byte defended leaks | **0** | `VERDICT.json` |
...
![**FIG-1.** CLRT distributions: broad, low-variance-defeating native READ/SELECT curves collapse to a single spike at 4 ms under defense.](figs/FIG-1_clrt_distribution.png){width=82%}
...
- **Size + segmentation normalization** — [49] → [28,21], 1280/1280 CRC/checksum-valid, 0 escapes.
