...
/* ===================== TWO-PIPE SPLIT (PIPE 0 half) =====================
 * Under -DTWO_PIPE_PIPE0 the protected OPERATE is NOT held on this pipe. It is stamped
 * with T0 in an internal xpipe header and routed across the internal cross-pipe loopback
 * to PIPE 1 (PORT_X1, dev_port>=128), which runs the BOR hold/release. Pipe 0 keeps the
 * WHOLE frozen RRC transaction (READ/SELECT ACK+response hold + PRE carve) and, for the
 * OPERATE, T0-ANCHORS reg_deadline/reg_tresp so the relay ACK/echo (later FRESH packets
 * back to pipe 0) are held to T0+A / T0+R, never re-anchored to the delayed release
 * (anti-subtraction §2). Compile pipe 0 with:  -DTWO_PIPE_PIPE0 -DBOR_NO_TOPJ  (reg_topj,
 * the qid3 reservoir and the OPERATE hold are pipe 1's, not pipe 0's). */
const PortId_t PORT_X1     = 9w144;  /* pipe-1 cross-pipe ENTRY (MAC loopback) */
const bit<16>  ETYPE_XPIPE = 16w0x88C2;  /* internal cross-pipe carrier (restored at release) */
...
/* ========================================================================== *
 *  ►►►► BOR — BOUNDED OPERATE RELEASE (compile probe additions) ◄◄◄◄
 *  This file is a byte-for-byte copy of the proven RRC kernel; every line added
 *  for BOR is tagged "BOR:". Spec: ../BOR_RRC_DESIGN.md. The proven ingress is
 *  already 12/12 stages with B0-15 / H0-15 / W0-15 all container-exhausted, so a
 *  clean placement-FAILURE is an acceptable, informative result of this probe.
 *
 *  BISECTION. The full BOR is active by default (no extra -D). To isolate the
 *  responsible feature, compile with one or more of the subtractive switches:
 *     -DBOR_NO_TOPJ      drop reg_topj (the 3rd deadline register), its expiry
 *                        table, the OPERATE T0+J arming and the qid3 deadline
 *                        drain; qid3 then drains on the pass budget only.
 *     -DBOR_NO_T0ANCHOR  the OPERATE arm no longer pre-arms reg_deadline/reg_tresp
 *                        to T0+A / T0+R (it disarms like READ/SELECT); removes the
 *                        bor_ack_cand / bor_resp_cand adds and the dec_op_arm write.
 *     -DBOR_NO_CODEBOOK  drop tbl_bor_params + tbl_bor_codebook; A/R/J are the
 *                        compile-time constants below instead of runtime-set.
 *     -DBOR_NO_TSGATE    drop the TCP-timestamp anti-subtraction violation counter.
 *  The OPERATE hold/release queues (qid2/qid3) are the irreducible BOR core and
 *  are always present; with every NO_* switch set the probe measures that core.
 * ========================================================================== */

/* BOR: two new strict-priority rungs below the RESP hold (see BOR_RRC_DESIGN §3).
 * qid3 = OPERATE blocker reservoir (drains at T0+J) · qid2 = held original OPERATE. */
const bit<5> QID_OP_BLOCK = 5w3;   /* Q_OP_BLOCK : OPERATE blocker reservoir  */
const bit<5> QID_OP_HOLD  = 5w2;   /* Q_OP_HOLD  : held original OPERATE       */

#ifdef SR1_TM_DISPATCH
/* SR1: compact internal TM-destination codes. Each loopback-enqueue site sets ONE of
 * these into meta.tm_dest; a single tbl_tm_dispatch at the end of ingress writes the
 * actual (port,qid,bypass_egress) once per packet. TMD_NONE = the D3_TO_FWD/DROP/SHAPE
 * paths that write ig_tm_md themselves (dispatch is a no-op for them). This collapses the
 * six bare-action TM tables (to_block/to_hold/to_resp_block/to_resp_hold/to_op_block/
 * to_op_hold) into one keyed dispatch table. Semantics-identical: the same qid/port that
 * each named action wrote is emitted, only from a single site. */
#define TMD_NONE       8w0
#define TMD_ACK_BLOCK  8w1   /* qid7 */
#define TMD_ACK_HOLD   8w2   /* qid6 */
#define TMD_RESP_BLOCK 8w3   /* qid5 */
#define TMD_RESP_HOLD  8w4   /* qid4 */
#define TMD_OP_BLOCK   8w5   /* qid3 */
#define TMD_OP_HOLD    8w6   /* qid2 */
#endif

/* BOR: blocker sub-role for the OPERATE reservoir, carried in ibspg_h.slot. */
const bit<8> SLOT_OP = 8w2;        /* OPERATE blocker token: qid3, checks reg_topj (T0+J) */

/* BOR: OPERATE deadline defaults (256 ns ticks, LOW BYTE ZERO so the ARMED marker
 * survives now+X). A = ACK release offset (T0+A), R = echo release offset (T0+R),
 * J = bounded request-hold (T0+J). Runtime-set by the codebook/params tables unless
 * -DBOR_NO_CODEBOOK. Constraints A>J+ackbound, R>J+respbound, R>=A are CP-validated. */
const bit<32> BOR_A_DEFAULT_TICKS = 32w0x001E8400;  /* ~2 ms  (placeholder)  */
const bit<32> BOR_R_DEFAULT_TICKS = 32w0x001E8400;  /* ~2 ms  (placeholder)  */
const bit<32> BOR_J_DEFAULT_TICKS = 32w0x000F4200;  /* ~1 ms  (placeholder)  */

/* BOR: new ctr_fresh slots (object size 32; baseline uses 0..17). */
const bit<8> CF_OP_HOLD        = 8w18;  /* OPERATE held in qid2 (request hold armed)   */
const bit<8> CF_OP_BYPASS      = 8w19;  /* OPERATE bypassed: concurrent / OFF / missing */
const bit<8> CF_OP_RETRANS_SUPP= 8w20;  /* retransmitted OPERATE while held: suppressed */
/* BOR: new ctr_deq slots (object size 16; baseline uses 0..7). */
const bit<8> CD_OP_LOOP        = 8w8;   /* qid3 OPERATE blocker re-enqueued            */
const bit<8> CD_OP_TERM_STALE  = 8w9;   /* qid3 token terminated: not this generation  */
const bit<8> CD_OP_TERM_DL     = 8w10;  /* qid3 token terminated: T0+J reached          */
const bit<8> CD_OP_TERM_TMO    = 8w11;  /* qid3 token terminated: fail-open horizon      */
const bit<8> CD_OP_RELEASE     = 8w12;  /* held OPERATE released to the relay (once)     */
...
#endif
    ethernet_h  eth;
#ifdef TWO_PIPE_PIPE0
    xpipe_h     xpipe;   /* TWO-PIPE: cross-pipe carrier, emitted only on the OPERATE to pipe 1 */
#endif
    ibspg_h     ib;
    ipv4_h      ipv4;
    tcp_h       tcp;
...
struct ig_meta_t {
    ...
    /* ---- BOR: OPERATE-request-hold state. ALL 16- or 32-bit so they land in the
     * EMPTY W32-47 / H32-47 MAU groups — the 8-bit B0-15 and 16-bit H0-15 and
     * 32-bit W0-15 ingress groups are already container-exhausted in the baseline. */
    bit<32> a_ticks;       /* BOR: ACK offset A  (T_ACK  = T0 + A), from bor params  */
    ...
    bit<32> topj_cand;     /* BOR: now_word + J = the armed T0+J word for the OPERATE */
    bit<32> age_topj;      /* BOR: now_word - reg_topj, for the qid3 blocker expiry   */
    bit<32> dl_val_topj;   /* BOR: PHV input 2 of reg_topj (DL_NO_WRITE = do not write)*/
    bit<16> expired_topj;  /* BOR: 1 = T0+J armed AND due (qid3 OP blocker only)      */
    bit<16> ts_opt_present;/* BOR: 1 = frame carried a TCP-timestamp-width option set */
#ifdef SR1_TM_DISPATCH
    bit<8>  tm_dest;       /* SR1: compact TM-destination code, dispatched once at end   */
#endif
#ifdef SR4_READINESS
    bit<8>  op_ready;      /* SR4: 1 = qid3 OPERATE reservoir confirmed resident (this gen) */
#endif
#ifdef SR3_CP_TS_GATE
    bit<8>  ts_cp_present; /* SR3: CP preflight flag - protected master flow negotiated TCP TS */
#endif
#if defined(PART4_RANDOM) || defined(PART4_SALTED)
    bit<8>  j_bucket;      /* PART4: leak-safe bounded bucket index -> per-flow delay profile */
#endif
#ifdef PART4_SALTED
    bit<8>  salt8;         /* PART4-B: secret salt read from reg_bor_salt (CP startup value)  */
#endif
...
parser IgParser(packet_in pkt,
    ...
    state start {
        ...
        /* BOR: zero the OPERATE-hold fields (matches implicit init_zero, so behaviour-
         * preserving; each is reassigned on its own path). */
        meta.a_ticks         = 32w0;
        meta.r_ticks         = 32w0;
        meta.arm_dl_val      = UNARMED_WORD;
        meta.arm_dl_val_resp = UNARMED_WORD;
        meta.j_ticks         = 32w0;
        meta.bor_ack_cand    = 32w0;
        meta.bor_resp_cand   = 32w0;
        meta.topj_cand       = 32w0;
        meta.age_topj        = 32w0;
        meta.dl_val_topj     = DL_NO_WRITE;
        meta.expired_topj    = 16w0;
        meta.ts_opt_present  = 16w0;
#ifdef SR1_TM_DISPATCH
        meta.tm_dest         = TMD_NONE;   /* SR1: default = FWD/DROP/SHAPE handle ig_tm_md */
#endif
#ifdef SR4_READINESS
        meta.op_ready        = 8w0;        /* SR4: reset each pass; set by qid3 confirm read */
#endif
#ifdef SR3_CP_TS_GATE
        meta.ts_cp_present   = 8w0;        /* SR3: default 0; CP sets via sess_master entry  */
#endif
#if defined(PART4_RANDOM) || defined(PART4_SALTED)
        meta.j_bucket        = 8w0;        /* PART4: leak-safe bucket index                  */
#endif
#ifdef PART4_SALTED
        meta.salt8           = 8w0;        /* PART4-B: secret salt                           */
#endif
        meta.dl_pre          = 32w0;
        meta.expired         = 8w0;
    ...
    /* BOR: data_offset 8 = 12 B of options = the Linux NOP,NOP,TCP-timestamp layout
     * (kind 8, len 10, + 2 NOP). The anti-subtraction claim (BOR_RRC_DESIGN §2) is
     * INVALID whenever the relay negotiates TCP timestamps, so mark it here and let the
     * MAU count a violation for the protected OPERATE flow. This does NOT block. */
    state opt12 {
        pkt.extract(hdr.tcp_opt12);
#if !defined(BOR_NO_TSGATE) && !defined(SR3_CP_TS_GATE)
        meta.ts_opt_present = 16w1;   /* data-offset inference; REMOVED under SR3 (not proof of kind 8) */
#endif
        transition parse_dnp3_dl;
    }
...
control Ingress(inout headers_t hdr,
...
#ifndef BOR_NO_TOPJ
    /* ================= BOR: reg_topj — the OPERATE request-hold deadline T0+J ========
     * Third deadline register, structurally identical to reg_deadline/reg_tresp. Armed
     * ONCE at the OPERATE ARM (T0+J, T0 = now at OPERATE); read as an AGE by the qid3
     * OPERATE-blocker on its loopback pass so the reservoir drains at T0+J and releases
     * the held original OPERATE. One RegisterAction does both: arm when dl_val_topj is a
     * real word, else return the age. PHV inputs: now_word, dl_val_topj — exactly 2.
     * ►► This is the PRIME no-fit suspect: it is a 3rd deadline register on a pipeline
     * whose reg_deadline already cannot co-locate (see Gate-2B history). */
...
#ifdef SR4_READINESS
    /* SR4: qid3 OPERATE-reservoir residency confirmation (BOR_RRC_DESIGN §3 readiness race).
     * A qid3 blocker token that completes a confirmed loop stamps the LIVE generation here;
     * the OPERATE arm reads it and holds ONLY when residency is proven for its own generation,
     * else it FAILS OPEN WITHOUT HOLDING (forwards immediately, never racing its reservoir).
     * Single 8-bit register, one confirm-write and one compare-read (both single-PHV-input). */
    Register<bit<8>, bit<1>>(1, 0) reg_op_ready;
    RegisterAction<bit<8>, bit<1>, bit<8>>(reg_op_ready) op_ready_confirm = {
        void apply(inout bit<8> v) { v = meta.cur_gen; }         /* mark resident-for-this-gen */
    };
    RegisterAction<bit<8>, bit<1>, bit<8>>(reg_op_ready) op_ready_read = {
        void apply(inout bit<8> v, out bit<8> rv) {
            if (v == meta.cur_gen) { rv = 8w1; } else { rv = 8w0; }
        }
    };
#endif
#ifndef BOR_NO_TSGATE
    /* BOR: the anti-subtraction violation counter (BOR_RRC_DESIGN §2). Non-zero means a
     * protected OPERATE flow carried TCP timestamps, so the T0-anchoring does NOT close
     * the timestamp leak channel and the design must NOT assert timestamp-safe behaviour.
     * A dedicated Counter (separate Stats-ALU object) so it is touched independently of
     * the ctr_fresh path that also fires on the OPERATE. */
    Counter<bit<64>, bit<8>>(1, CounterType_t.PACKETS) ctr_ts_viol;
#endif
    ...
    /* BOR: the OPERATE hold queue (qid2) and the OPERATE blocker reservoir (qid3).
     * The original OPERATE is queue-resident in qid2, starved by the qid3 reservoir
     * until T0+J; only qid3 blocker tokens loop. Symmetric to the ACK/RESP rungs. */
    action to_op_hold() {
        ig_tm_md.ucast_egress_port = PORT_L;
        ig_tm_md.qid               = QID_OP_HOLD;   /* qid2 */
        ig_tm_md.bypass_egress     = 1w1;
    }
    action to_op_block() {
        ig_tm_md.ucast_egress_port = PORT_L;
        ig_tm_md.qid               = QID_OP_BLOCK;  /* qid3 */
        ig_tm_md.bypass_egress     = 1w1;
    }
    /* SR1: enqueue-site macros. Under SR1_TM_DISPATCH a site sets a compact code (folded
     * into the leaf that already runs there, e.g. the hdr.ib writes or the seq decrement),
     * and tbl_tm_dispatch writes the real (port,qid,bypass) ONCE at the end — replacing six
     * bare-action logical tables with one keyed table. Under !SR1 the macros expand to the
     * original bare-action calls, byte-for-byte the full-BOR probe. */
#ifdef SR1_TM_DISPATCH
    #define TO_BLOCK()       { meta.tm_dest = TMD_ACK_BLOCK; }
    #define TO_HOLD()        { meta.tm_dest = TMD_ACK_HOLD; }
    #define TO_RESP_BLOCK()  { meta.tm_dest = TMD_RESP_BLOCK; }
    #define TO_RESP_HOLD()   { meta.tm_dest = TMD_RESP_HOLD; }
    #define TO_OP_BLOCK()    { meta.tm_dest = TMD_OP_BLOCK; }
    #define TO_OP_HOLD()     { meta.tm_dest = TMD_OP_HOLD; }
#else
    #define TO_BLOCK()       { to_block(); }
    #define TO_HOLD()        { to_hold(); }
    #define TO_RESP_BLOCK()  { to_resp_block(); }
    #define TO_RESP_HOLD()   { to_resp_hold(); }
    #define TO_OP_BLOCK()    { to_op_block(); }
    #define TO_OP_HOLD()     { to_op_hold(); }
#endif
    /* ================= THE INLINE LEVER =================================
...
#ifdef SR2_FOLD_PARAMS
    /* SR2: A/R/default-J folded into the SAME keyless tbl_params default action. The BOR
     * deadline params are per-deployment constants with no key and no dependency that
     * forbids co-residence with the timing params, so one keyless table carries all nine
     * knobs and tbl_bor_params (a second keyless logical table) is removed entirely. The
     * per-flow J override still comes from the keyed tbl_bor_codebook after this. */
    action set_params(bit<32> d_ticks, bit<32> read_len, bit<32> budget,
                      bit<8> mode, bit<32> da_dr, bit<16> shape_enable,
                      bit<32> a_ticks, bit<32> r_ticks, bit<32> j_ticks) {
        meta.seq_m        = d_ticks;
        ...
        meta.j_ticks      = j_ticks;   /* default J; overridden by the codebook when enabled */
    }
    table tbl_params {
        actions = { set_params; }
        default_action = set_params(D_DEFAULT_TICKS, READ_LEN_DEFAULT, BUDGET_DEFAULT,
                                    MODE_D3_ACK, D_DEFAULT_TICKS, 16w0,
                                    BOR_A_DEFAULT_TICKS, BOR_R_DEFAULT_TICKS, BOR_J_DEFAULT_TICKS);
        size = 1;
    }
#else
    action set_params(bit<32> d_ticks, bit<32> read_len, bit<32> budget,
    ...
    /* ================= BOR level 0: OPERATE deadline params + delay codebook =========
     * A / R are per-deployment OPERATE deadline offsets, DISTINCT from the READ/SELECT
     * D_A / da_dr (BOR_RRC_DESIGN §1). J is the bounded request-hold, selected by a
     * control-plane codebook keyed on FLOW OWNERSHIP (never on the DNP3 app-sequence, so
     * an observer cannot learn and subtract the mapping — §2/§7). Both keyless/small,
     * rewritten with default_entry_set exactly like tbl_params. */
    action set_bor_params(bit<32> a_ticks, bit<32> r_ticks, bit<32> j_ticks) {
        meta.a_ticks = a_ticks;
        meta.r_ticks = r_ticks;
        meta.j_ticks = j_ticks;   /* default J; overridden by the codebook when enabled */
    }
    table tbl_bor_params {
        ...
        default_action = set_bor_params(BOR_A_DEFAULT_TICKS, BOR_R_DEFAULT_TICKS,
                                        BOR_J_DEFAULT_TICKS);
        size = 1;
    }
#endif
#ifndef BOR_NO_CODEBOOK
    /* the bounded delay codebook: flow ownership (relay dport) -> J. The control plane
     * installs the admissible {0,2,4,6,8,10,12} ms entries with per-profile weights; the
     * eventual selector draws a leak-safe J and installs it here (a Tofino random extern
     * or low hw-timestamp bits + a secret salt), NEVER a DNP3-sequence-derived value. */
    action set_bor_j(bit<32> j_ticks) { meta.j_ticks = j_ticks; }
    table tbl_bor_codebook {
        key = { hdr.tcp.dst_port : ternary; }   /* flow ownership proxy (relay port) */
        actions = { set_bor_j; NoAction; }
...
#if defined(PART4_RANDOM) || defined(PART4_SALTED)
    /* ================= PART 4: LEAK-SAFE J SELECTOR ==============================
     * `tcp.dst_port -> fixed J` (the codebook above) is only a PROFILE lookup: it shifts J
     * per flow, it does not CONVOLVE it, so a fixed-J MVP does NOT mitigate the Formby
     * physical-operation-time fingerprint. This selector draws a leak-safe bounded bucket
     * index per OPERATE and maps it, via a per-flow profile table, to an admissible J.
     * The bucket source is NEVER the public DNP3 application sequence (§2/§7). */
    action set_j_from_bucket(bit<32> j_ticks) { meta.j_ticks = j_ticks; }
    table tbl_bor_jbucket {
        /* bounded buckets -> admissible J; the CP installs {0,2,4,6,8,10,12} ms with the
         * per-flow profile's weights (how many buckets map to each J = the profile). The key
         * is the LOW NIBBLE of the leak-safe index (16 buckets) so the selector actions stay
         * single-stage (no post-XOR/post-RNG mask). */
        key     = { meta.j_bucket[3:0] : exact; }
        actions = { set_j_from_bucket; NoAction; }
        const default_action = NoAction();
        size    = 16;
    }
#endif
#ifdef PART4_RANDOM
    /* PART4-A: the Tofino RNG extern draws a uniform value each OPERATE; the bucket table
     * keys on its low nibble. Randomized per-request J (convolution), not a per-flow shift. */
    Random<bit<8>>() bor_rng;
    action bor_pick_random() { meta.j_bucket = bor_rng.get(); }
#endif
#ifdef PART4_SALTED
    /* PART4-B: a secret-salted hardware-timestamp selector. bucket = low byte of the ingress
     * hw timestamp XOR a per-deployment secret salt (CP-installed at startup, read once);
     * the bucket table keys on the low nibble. The salt keeps the mapping unknown to an
     * observer who can see arrival times. One XOR -> single-stage action. */
    Register<bit<8>, bit<1>>(1, 0) reg_bor_salt;    /* CP writes a secret 8-bit salt */
    RegisterAction<bit<8>, bit<1>, bit<8>>(reg_bor_salt) salt_read = {
        void apply(inout bit<8> v, out bit<8> rv) { rv = v; }
    };
    action bor_pick_salted() { meta.j_bucket = meta.ts32[7:0] ^ meta.salt8; }
#endif

#ifdef SR1_TM_DISPATCH
    /* SR1: the ONE final TM-dispatch table. Keyed on the compact meta.tm_dest code set by
     * the enqueue sites; writes the real loopback (port,qid,bypass_egress) exactly once.
     * Each action is byte-identical to the named action it replaces (same PORT_L + qid).
     * TMD_NONE -> tmd_noop: the FWD/DROP/SHAPE paths already wrote ig_tm_md, so dispatch
     * leaves them untouched. Applied once, after the ACT block. */
    action tmd_ack_block()  { ig_tm_md.ucast_egress_port = PORT_L; ig_tm_md.qid = QID_ACK_BLOCK;  ig_tm_md.bypass_egress = 1w1; }
    action tmd_ack_hold()   { ig_tm_md.ucast_egress_port = PORT_L; ig_tm_md.qid = QID_ACK_HOLD;   ig_tm_md.bypass_egress = 1w1; }
    action tmd_resp_block() { ig_tm_md.ucast_egress_port = PORT_L; ig_tm_md.qid = QID_RESP_BLOCK; ig_tm_md.bypass_egress = 1w1; }
    action tmd_resp_hold()  { ig_tm_md.ucast_egress_port = PORT_L; ig_tm_md.qid = QID_RESP_HOLD;  ig_tm_md.bypass_egress = 1w1; }
    action tmd_op_block()   { ig_tm_md.ucast_egress_port = PORT_L; ig_tm_md.qid = QID_OP_BLOCK;   ig_tm_md.bypass_egress = 1w1; }
    action tmd_op_hold()    { ig_tm_md.ucast_egress_port = PORT_L; ig_tm_md.qid = QID_OP_HOLD;    ig_tm_md.bypass_egress = 1w1; }
    action tmd_noop()       { }
    table tbl_tm_dispatch {
        key     = { meta.tm_dest : exact; }
        actions = { tmd_ack_block; tmd_ack_hold; tmd_resp_block; tmd_resp_hold;
                    tmd_op_block; tmd_op_hold; tmd_noop; }
        const entries = {
            TMD_ACK_BLOCK  : tmd_ack_block();
            TMD_ACK_HOLD   : tmd_ack_hold();
            TMD_RESP_BLOCK : tmd_resp_block();
            TMD_RESP_HOLD  : tmd_resp_hold();
            TMD_OP_BLOCK   : tmd_op_block();
            TMD_OP_HOLD    : tmd_op_hold();
        }
        const default_action = tmd_noop();
        size = 8;
    }
#endif
...
#ifdef SR3_CP_TS_GATE
    /* SR3: the CP-set per-flow timestamp flag is carried as action data on the SAME session
     * entry that already keys the protected master flow (no new table). The control plane,
     * having observed the SYN/SYN-ACK preflight, installs ts_present=1 iff this flow
     * negotiated TCP timestamps. data_offset==8 is NOT proof of option kind 8, so the data
     * plane no longer infers TS from the option width (parser writes removed below). */
    action sess_master(bit<8> ts_present) {
...
#if !defined(BOR_NO_T0ANCHOR) || !defined(BOR_NO_TOPJ)
    /* BOR level 2: the OPERATE armed-word candidates, T0-anchored (T0 = now at the
     * OPERATE). now_word is level 1; a/r/j are level 0 — one add each, exactly the
     * build_cand idiom. Kept independent so no add feeds another (no intra-action
     * multi-stage dependency). Consumed only on the OPERATE arm path. */
    action build_bor_cands() {
#ifndef BOR_NO_T0ANCHOR
        meta.bor_ack_cand  = meta.now_word + meta.a_ticks;
        meta.bor_resp_cand = meta.now_word + meta.r_ticks;
#endif
#ifndef BOR_NO_TOPJ
        meta.topj_cand     = meta.now_word + meta.j_ticks;
#endif
    }
    table tbl_build_bor_cands {
        actions = { build_bor_cands; }
        const default_action = build_bor_cands();
        size = 1;
    }
#endif

#ifdef PIPE0_ARM_FOLD
    /* PIPE0_ARM_FOLD: func_code-keyed precompute of the fresh-ARM deadline words, at level 2.
     * OPERATE -> T0+A / T0+R (T0-anchored, one add each); READ/SELECT/default -> UNARMED_WORD.
     * dec_arm_fresh then writes dl_val = arm_dl_val in the decode (level 3), matching the ACK
     * arm's build_cand->dec_ack_arm->reg_deadline shape exactly, so no write-after-write and
     * no extra stage. Only the func_code (parser-extracted) is the key; no MAU arithmetic on it. */
    action arm_fold_operate() {
        meta.arm_dl_val      = meta.now_word + meta.a_ticks;   /* T0 + A */
        meta.arm_dl_val_resp = meta.now_word + meta.r_ticks;   /* T0 + R */
    }
    action arm_fold_none() {
        meta.arm_dl_val      = UNARMED_WORD;
        meta.arm_dl_val_resp = UNARMED_WORD;
    }
    table tbl_arm_fold {
        key = { hdr.dnp3_app.func_code : exact; }
        actions = { arm_fold_operate; arm_fold_none; }
        const entries = { DNP3_FC_OPERATE : arm_fold_operate(); }
        const default_action = arm_fold_none();
        size = 4;
    }
#endif
...
#ifdef PIPE0_ARM_FOLD
    /* PIPE0_ARM_FOLD: fold the OPERATE T0-anchor into the SAME shape as the ACK arm
     * (dec_ack_arm writes dl_val = dl_cand). arm_dl_val/arm_dl_val_resp are precomputed by
     * the func_code-keyed tbl_arm_fold at level 2 (T0+A / T0+R for an OPERATE, UNARMED else),
     * so this is ONE write to dl_val in the decode — no write-after-write, no extra stage. */
    action dec_arm_fresh()  { meta.dl_val = meta.arm_dl_val; meta.dl_val_resp = meta.arm_dl_val_resp; meta.verdict = V_ARM_FRESH; }
#else
...
#ifndef BOR_NO_TOPJ
    /* BOR: OPERATE request-hold expiry (T0+J), symmetric to the ACK/RESP expiry — same
     * whole-container armed-and-due mask on age_topj. An unarmed reg_topj (before the
     * OPERATE arrives, or after a READ/SELECT that never sets dl_val_topj) can never
     * read as expired, so the qid3 tokens loop harmlessly until T0+J is armed. */
    ...
    table tbl_topj_expiry {
        ...
        const entries = {
            (32w0x00000000 &&& 32w0x800000FF) : mark_expired_topj();
        }
        size = 2;
    }
#endif
    ...
    apply {
        if (meta.port_ok == 8w0) {
            ...
            tbl_params.apply();                                  /* D, read_len, B (+A,R,J under SR2) */
#ifndef SR2_FOLD_PARAMS
            tbl_bor_params.apply();                              /* BOR: A, R, default J */
#endif
#ifndef BOR_NO_CODEBOOK
            tbl_bor_codebook.apply();                            /* BOR: per-flow J      */
#endif
#ifdef PART4_RANDOM
            bor_pick_random();                                   /* PART4: RNG -> bucket */
            tbl_bor_jbucket.apply();                             /* PART4: bucket -> J    */
#endif
#ifdef PART4_SALTED
            meta.salt8 = salt_read.execute(0);                   /* PART4: read secret salt (SALU) */
            bor_pick_salted();                                   /* PART4: tstamp^salt -> bucket   */
            tbl_bor_jbucket.apply();                             /* PART4: bucket -> J    */
#endif
...
#if !defined(BOR_NO_T0ANCHOR) || !defined(BOR_NO_TOPJ)
            tbl_build_bor_cands.apply();     /* BOR: T0+A, T0+R, T0+J candidates */
#ifdef PIPE0_ARM_FOLD
            tbl_arm_fold.apply();            /* PIPE0_ARM_FOLD: precompute fresh-ARM dl words (func_code-keyed) */
#endif
#endif
            ...
            /* ---------- BOR: T0-anchor the OPERATE deadlines ----------
             * tbl_state_decode gave the OPERATE (a fresh master ROLE_ARM) verdict
             * V_ARM_FRESH via dec_arm_fresh, which DISARMED reg_deadline/reg_tresp
             * (dl_val = dl_val_resp = UNARMED_WORD). For an OPERATE we instead ARM them
             * to T0+A / T0+R now (T0 = now at the OPERATE), and arm reg_topj to T0+J, so
             * the ACK/echo release instants are T0-anchored and NEVER re-anchored to the
             * delayed release or the relay ACK (anti-subtraction, §2). func_code == OPERATE
             * distinguishes it from READ/SELECT, which the decode key cannot. This is a
             * write-after-write on dl_val at level 3, one of BOR's real added dependencies. */
            if (meta.pkt_class == CLASS_ARM && meta.verdict == V_ARM_FRESH
                    && hdr.dnp3_app.func_code == DNP3_FC_OPERATE
                    && meta.mode != MODE_OFF && meta.mode != MODE_FAIL_OPEN) {
#if !defined(BOR_NO_T0ANCHOR) && !defined(PIPE0_ARM_FOLD)
                /* baseline SR-probe realization: write-after-write on dl_val (costs +1 stage).
                 * PIPE0_ARM_FOLD replaces this by folding the arm value into dec_arm_fresh. */
                meta.dl_val      = meta.bor_ack_cand;    /* reg_deadline := T0 + A */
                meta.dl_val_resp = meta.bor_resp_cand;   /* reg_tresp    := T0 + R */
#endif
#ifndef BOR_NO_TOPJ
...
#ifndef BOR_NO_TOPJ
            /* BOR: reg_topj — arms to T0+J on the OPERATE arm (dl_val_topj = topj_cand),
             * else returns the age for the qid3 OPERATE blocker's expiry test. One access
...
#endif
#ifdef SR4_READINESS
            /* SR4: BOTH accesses to reg_op_ready live here at level 5 (mutually exclusive
             * branches, one pipeline depth) so the register's two RegisterActions co-locate
             * in one stage. A dequeued LIVE qid3 token confirms residency for the live
             * generation; a fresh OPERATE arm reads whether residency is proven for its own
             * generation, and the ACT block below holds vs. fails open on that flag. */
            if (meta.dequeued == 8w1 && meta.role == ROLE_BLOCK && hdr.ib.slot == SLOT_OP
                    && (meta.verdict == V_BLOCK_LIVE || meta.verdict == V_BLOCK_PENDING)) {
                op_ready_confirm.execute(0);                 /* qid3 resident-for-this-gen */
            } else if (meta.dequeued == 8w0 && meta.role == ROLE_ARM
                    && hdr.dnp3_app.func_code == DNP3_FC_OPERATE) {
                meta.op_ready = op_ready_read.execute(0);    /* proven-resident? (0/1)     */
            }
#endif
#if defined(SR3_CP_TS_GATE)
            /* SR3: the anti-subtraction violation is driven by the CP preflight flag
             * (sess_master action data), NOT by a data-plane data_offset inference. The DP
             * reads the CP flag only; data_offset==8 is not proof of TCP option kind 8. */
            if (meta.ts_cp_present == 8w1 && meta.dir == DIR_MASTER
                    && meta.role == ROLE_ARM
                    && hdr.dnp3_app.func_code == DNP3_FC_OPERATE) {
                ctr_ts_viol.count(8w0);
            }
#elif !defined(BOR_NO_TSGATE)
            /* BOR: record the anti-subtraction violation — a protected OPERATE flow that
             * carries TCP timestamps. Fires at most once per packet, on its own Counter. */
            if (meta.ts_opt_present == 16w1 && meta.dir == DIR_MASTER
                    && meta.role == ROLE_ARM
                    && hdr.dnp3_app.func_code == DNP3_FC_OPERATE) {
                ctr_ts_viol.count(8w0);
            }
            ...
            if (meta.dequeued == 8w0) {
                ...
                if (meta.role == ROLE_CLONE) {
                    ...
                    if (meta.is_pktgen == 8w1) {
                         ...
                         * reach. */
                        /* BOR: THREE reservoirs now share one burst — packet_id[7:6]
                         * selects the slot, [15:8] must be 0 (id < 256):
                         *   0..63   -> ACK  blocker qid7 (slot ACK)
                         *   64..127 -> RESP blocker qid5 (slot RESP)
                         *   128..191-> OP   blocker qid3 (slot OP)   [BOR]
                         *   >=192   -> invalid, drop. */
                        if (meta.txn_active == 8w1 && hdr.pgen.packet_id[15:8] != 8w0) {
                            D3_DROP()                       /* invalid packet_id (>= 256) */
                            ctr_fresh.count(CF_PKTGEN_DROP);
                        } else if (meta.txn_active == 8w1 && hdr.pgen.packet_id[7:6] == 2w0) {
                            /* packet_id 0..63 -> ACK blocker, first-enqueue qid7. */
                        ...
                        } else if (meta.txn_active == 8w1 && hdr.pgen.packet_id[7:6] == 2w1) {
                            /* packet_id 64..127 -> RESPONSE blocker, first-enqueue qid5. */
                            hdr.ib.role = ROLE_BLOCK;
                            ...
                            ctr_fresh.count(CF_PKTGEN_ADMIT);
                            meta.ev_first_block = 8w1;
#ifndef TWO_PIPE_PIPE0
                        } else if (meta.txn_active == 8w1 && hdr.pgen.packet_id[7:6] == 2w2) {
                            /* BOR: packet_id 128..191 -> OPERATE blocker, first-enqueue qid3. */
                            hdr.ib.role = ROLE_BLOCK;
                            hdr.ib.slot = SLOT_OP;
                            hdr.ib.gen  = meta.cur_gen;
                            hdr.ib.seq  = meta.budget_init;
                            TO_OP_BLOCK()                     /* qid3 OPERATE reservoir */
                            ctr_fresh.count(CF_PKTGEN_ADMIT);
                            meta.ev_first_block = 8w1;
#endif
                        } else if (meta.txn_active == 8w1) {
                            D3_DROP()                       /* packet_id 192..255: invalid */
                            ctr_fresh.count(CF_PKTGEN_DROP);
                        } else {
                    ...
                    if (meta.verdict == V_RESP && meta.txn_active == 8w1 &&
                         ...
                         * RRC: in OFF mode a fresh eligible 49 B response reaches here
                         * (no active txn) and is carved immediately when shaping is armed;
                         * the payload49 + shape_enable gate keeps every other bypass unicast. */
                        RRC_RESP_FWD()
                    ...
                    if (meta.verdict == V_ACK_ARM &&
                        ...
                        ctr_fresh.count(CF_ACK_REJECT);
                               ...
                               && hdr.dnp3_app.func_code == DNP3_FC_OPERATE
                               && meta.mode != MODE_OFF && meta.mode != MODE_FAIL_OPEN) {
                    /* ============ BOR: THE OPERATE-REQUEST-HOLD PHASE ============
                     * The OPERATE is master->relay, so unlike the held ACK/RESPONSE it must
                     * be released toward the RELAY, not the master. It is HELD in qid2 and
                     * released exactly once when the qid3 reservoir drains at T0+J (handled
                     * on the dequeued OPERATE path below). Its ACK/echo deadlines were armed
                     * to T0+A / T0+R at level 3, so WAIT_ACK is entered T0-anchored. */
                    if (meta.verdict == V_ARM_FRESH) {
#ifdef TWO_PIPE_PIPE0
                        /* ►► TWO-PIPE: DO NOT HOLD. Stamp T0 into the xpipe header and route
                         * the OPERATE across the internal cross-pipe loopback to PIPE 1
                         * (PORT_X1), which runs the BOR hold/release on its own reservoir.
                         * bypass_egress=1 so pipe-1 EGRESS is skipped and the xpipe-tagged
                         * frame reaches pipe-1 INGRESS intact. arm_clone() STILL seeds THIS
                         * pipe's qid7/qid5 reservoirs so the relay's ACK/echo (later fresh
                         * packets to pipe 0) are held to the T0-anchored reg_deadline/reg_tresp
                         * armed at level 3. Exactly ONE OPERATE crosses (single ucast copy). */
                        hdr.xpipe.setValid();
                        ...
                        ig_tm_md.bypass_egress     = 1w1;
                        arm_clone();
                        ctr_fresh.count(CF_OP_HOLD);   /* slot reused: OPERATE admitted + crossed */
#else
                        TO_OP_HOLD()                 /* hold the ORIGINAL, byte-identical, qid2 */
                        arm_clone();                  /* seed qid7 + qid5 + qid3 reservoirs      */
                        ctr_fresh.count(CF_OP_HOLD);
#endif
                    } else if (meta.verdict == V_ARM_DUP) {
                        /* an EXACT retransmission of an OPERATE already held for THIS
                         * generation. Forwarding it delivers a SECOND OPERATE to the relay
                         * (a second physical operation); a second qid2 copy would
                         * double-release. So it is DROPPED — the one held copy still
                         * releases exactly once (failure table: retransmit-while-held). */
                        D3_DROP()
                        ctr_fresh.count(CF_OP_RETRANS_SUPP);
                    } else {
                        /* CONCURRENT: a different transaction is already live. Forward the
                         * OPERATE UNPROTECTED to the relay and count (scope limit: one
                         * active transaction), never hold it behind another. */
                        D3_TO_FWD()
                        ctr_fresh.count(CF_OP_BYPASS);
                    }

                } else if (meta.pkt_class == CLASS_ARM) {
                    /* a real DNP3 READ / SELECT (or an OFF/FAIL_OPEN OPERATE, a true
...
#ifndef TWO_PIPE_PIPE0
                if (meta.role == ROLE_BLOCK && hdr.ib.slot == SLOT_OP) {
                    /* BOR: the qid3 OPERATE blocker reservoir. Symmetric to the ACK/RESP
                     * reservoirs, gated on the OPERATE request-hold deadline (expired_topj
                     * from reg_topj = T0+J). When it drains, the qid2 held original OPERATE
                     * becomes eligible and is released to the relay (dequeued-OPERATE path
                     * below). Termination priority stale > deadline > budget. */
                    if (meta.verdict != V_BLOCK_LIVE && meta.verdict != V_BLOCK_PENDING) {
                        ...
                        D3_DROP()                            /* T0+J reached: release OPERATE */
                        ctr_deq.count(CD_OP_TERM_DL);
                        meta.ev_block_term = 8w1;
#endif
                    } else if (meta.budget_zero == 8w1) {
                        D3_DROP()                            /* fail-open horizon (release)   */
                        ctr_deq.count(CD_OP_TERM_TMO);
                        meta.ev_block_term = 8w1;
                    } else {
                        hdr.ib.seq = hdr.ib.seq - 32w1;
                        TO_OP_BLOCK()                       /* re-enqueue qid3 */
                        /* SR4: residency confirmation for reg_op_ready is done at level 5
                         * (co-located with the read), not here, to satisfy register-stage
                         * co-location on the over-full pipeline. */
                        ctr_deq.count(CD_OP_LOOP);
                    }

                } else if (meta.role == ROLE_BLOCK && meta.is_resp_blk == 8w1) {
#else
                /* TWO-PIPE: no qid3/qid2 OPERATE reservoir on pipe 0 (it lives on pipe 1). */
                if (meta.role == ROLE_BLOCK && meta.is_resp_blk == 8w1) {
                    ...
                    /* ►► RRC: THE TIMING-RELEASE EVENT ITSELF INVOKES THE SIZE OPERATION.
                     * This is the deadline-release pass (D2/D3/D4). A shape-armed, eligible
                     * 49 B response replicates into the PRE carve group here instead of a
                     * unicast dp9 copy; everything else forwards byte-identically as before.
                     * payload49 was re-derived by the parser on this dequeued pass. */
                    RRC_RESP_FWD()
                    ...
                    if (meta.expired_resp == 8w1) { ctr_deq.count(CD_RELEASE_DEADLINE); }
                               ...
                               && hdr.dnp3_app.func_code == DNP3_FC_OPERATE) {
                    /* ►► BOR: THE OP_RELEASE PASS. The qid3 reservoir drained at T0+J, so
                     * the held original OPERATE (qid2) is back on dp8. Forward it to the
                     * RELAY (dp64), byte-identically and EXACTLY ONCE (qid2 held a single
                     * copy — it does not loop). The ACK/echo deadlines were armed to T0+A /
                     * T0+R at the OPERATE arm, so the transaction is now in WAIT_ACK with no
                     * re-anchoring. No marker byte is added or stripped: byte identity holds.
                     * This inlines the forward (no named action) so it does not spend an
                     * extra logical table ID, per the inline-lever discipline. */
                    ig_tm_md.ucast_egress_port = PORT_RELAY;
                    ig_tm_md.qid               = QID_FWD;
                    ig_tm_md.bypass_egress     = 1w0;
                    ctr_deq.count(CD_OP_RELEASE);
#endif
                } else {
                    D3_DROP()   /* nothing else may loop back (pipe 0 never holds the OPERATE) */
                }
            }
...
#ifdef SR1_TM_DISPATCH
            /* SR1: single TM-destination dispatch, once per packet. All meta.tm_dest writes
             * completed in the ACT block above; nothing after this re-writes ig_tm_md. */
            tbl_tm_dispatch.apply();
#endif
#ifdef D3_TS_INTERNAL
...
control IgDeparser(packet_out pkt,
    ...
    apply {
        ...
        pkt.emit(hdr.eth);
#ifdef TWO_PIPE_PIPE0
        pkt.emit(hdr.xpipe);   /* TWO-PIPE: valid only on the OPERATE crossing to pipe 1 */
#endif
        pkt.emit(hdr.ib);
        pkt.emit(hdr.ipv4);
        pkt.emit(hdr.tcp);
...
parser EgParser(packet_in pkt, out headers_t hdr, out eg_meta_t m,
    ...
    state parse_eth {
        pkt.extract(hdr.eth);
        transition select(hdr.eth.etype) { ETHERTYPE_IPV4 : parse_ipv4; default : accept; }
    }
    state parse_ipv4 {
        pkt.extract(hdr.ipv4);
