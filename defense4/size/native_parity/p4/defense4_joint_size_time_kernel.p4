...
/* ======================================================================== *
 *  EGRESS — JOINT SIZE+TIME shaping. Native-parity CRC-boundary SPLIT layered
 *  onto the frozen caseA TIMING egress (a byte-preserving pass-through).
 *  Tofino-1 / TNA. COMPILE PROBE (software/model only — NOT silicon-validated).
 * ------------------------------------------------------------------------ *
 * ONE UNIFIED PRIMITIVE, ONE PREDICATE. `do_shape` is set by a SINGLE table
 * `t_shape` from exactly three facts about the packet — response DIRECTION
 * (src L4 port == DNP3 20000 -> DIR_OUT), owner FLOW (t_policy exact 5-tuple),
 * and native SIZE == target (ip.total_len == 89 -> 49 B on wire) — and NOTHING
 * about DNP3 type (READ G10/G30 vs SBO G12 are indistinguishable to it). A 49 B
 * READ response and a 49 B SBO echo hit the identical code path.
 *
 * do_shape drives the size transform HERE, and it is the SAME logical predicate
 * (owner response flow) that the frozen caseA INGRESS uses to hold the response
 * in the recirculation/hold ring and RELEASE it toward the master at deadline D.
 * The split can NEVER fire independently of that release: caseA sets
 * bypass_egress=1 on every held / recirculating pass, so a packet reaches THIS
 * egress ONLY on the deadline-release pass to dp9 (master). Timing normalization
 * (a) and size/segment normalization (b) therefore both hang off the one gate.
 *
 * SPLIT (fixed 2-way, egress_rid-selected; byte-preserving, 0 seq/ack ledger).
 *   On the release pass (egress_rid==0 SOURCE), a do_shape response emits the
 *   whole frame to a 2-node mcast group and drops its own unicast (drop_ctl bit0).
 *   Replicas carve on the CRC-block boundary cut=28 (header block + blk0):
 *     rid==1 -> window0 = frame[0..28)   seq=base      total_len 68  (PSH/FIN cleared)
 *     rid==2 -> window1 = frame[28..49)  seq=base+28   total_len 61  (final segment)
 *   join(window0,window1) == frame exactly; the master reassembles by seq.
 *   IPv4 total_len + IPv4/TCP checksums recomputed per window. NO ledger, NO state.
 *
 * NON-do_shape traffic (wrong size, non-owner, non-DNP3, SYN/RST, TCP/IP options,
 *   fragments) is a BYTE-PRESERVING pass-through: parsed headers + residual are
 *   re-emitted with the original checksums (translated stays 0), exactly as the
 *   caseA egress does. So forwarding + timing are byte-for-byte caseA behaviour.
 *
 * REPLICATION (mirror -> multicast group, per-copy egress_rid, source drop) is
 *   COMPILE-verified only; its RUNTIME behaviour on silicon is UNPROVEN (label).
 * ======================================================================== */

/* egress-only constants (caseA already defines ETHERTYPE_IPV4/IP_PROTO_TCP/DNP3_START/DIR_*) */
const bit<16> DNP3_L4_PORT = 16w20000;       /* outstation listen port -> DIR_OUT       */
const bit<8>  POL_NONE  = 8w0;
const bit<8>  POL_SPLIT = 8w1;
const bit<8>  SHP_CUT   = 8w28;              /* the CRC-block boundary: header(10)+blk0(18) */
const bit<16> TL_SHAPE  = 16w89;             /* target native size 49 B => ip.total_len 89 */
const bit<16> W0_TOTAL  = 16w68;             /* window0 ip.total_len = 40 + 28              */
const bit<8>  CLR_PSH_FIN = 8w0xF6;          /* clear PSH(0x08)+FIN(0x01) on the earlier seg */
typedef bit<10> mirror_sess_t;
const mirror_sess_t SHAPE_SESSION = 10w9;    /* CP binds -> mcast group {rid1,rid2} -> dp9  */
const bit<3> MIRR_SHAPE = 3w1;

struct eg_meta_t {
    bit<8>  dir;
    bit<32> nm_ip; bit<32> nr_ip; bit<16> nm_pt; bit<16> nr_pt;
    bit<8>  eligible;     /* IPv4/TCP, ihl==5, not fragmented, dofs==5 */
    bit<8>  size49;       /* ip.total_len == 89 (the 49 B target)      */
    bit<8>  is_syn; bit<8> is_rst;
    bit<8>  owner;        /* exact protected-flow match                */
    bit<8>  policy;       /* POL_NONE / POL_SPLIT                       */
    bit<8>  do_shape;     /* THE single predicate (dir+owner+size, no type) */
...
parser EgParser(packet_in pkt, out headers_t hdr, out eg_meta_t m,
    ...
    state start {
        pkt.extract(eg);
        m.dir = DIR_MASTER; m.nm_ip = 32w0; m.nr_ip = 32w0; m.nm_pt = 16w0; m.nr_pt = 16w0;
        m.eligible = 8w0; m.size49 = 8w0; m.is_syn = 8w0; m.is_rst = 8w0;
        m.owner = 8w0; m.policy = POL_NONE; m.do_shape = 8w0; m.rid = 16w0;
    ...
    state parse_ipv4 {
        pkt.extract(hdr.ipv4);
        /* ihl==5, proto TCP, and (MF==0 && frag==0) tolerating DF — one masked select */
        transition select(hdr.ipv4.protocol, hdr.ipv4.ihl, hdr.ipv4.flags_frag) {
            (IP_PROTO_TCP, 4w5, 16w0x0000 &&& 16w0xBFFF) : parse_tcp;
            default : accept;
        }
    ...
    state parse_tcp {
        pkt.extract(hdr.tcp);
        transition select(hdr.tcp.data_offset) { 4w5 : mark_elig; default : accept; }
    }
    state mark_elig { m.eligible = 8w1; transition parse_dl; }
    state parse_dl {
        pkt.extract(hdr.dnp3_dl);
        transition select(hdr.dnp3_dl.start) { DNP3_START : parse_b0; default : accept; }
    }
    state parse_b0 { pkt.extract(hdr.blk0); transition parse_b1; }
    state parse_b1 {
        pkt.extract(hdr.blk1);
        transition select(hdr.ipv4.total_len) { TL_SHAPE : parse_res3; default : accept; }
    }
    state parse_res3 { pkt.extract(hdr.res3); transition sz_shape; }
    state sz_shape { m.size49 = 8w1; transition accept; }
}
...
control Egress(inout headers_t hdr, inout eg_meta_t m,
    ...
    /* direction + canonical bidirectional flow fold (keyed on the DNP3 L4 port) */
    action e_fold_master() { m.dir = DIR_MASTER; m.nm_ip = hdr.ipv4.src_addr; m.nm_pt = hdr.tcp.src_port; m.nr_ip = hdr.ipv4.dst_addr; m.nr_pt = hdr.tcp.dst_port; }
    action e_fold_out()    { m.dir = DIR_OUT;    m.nm_ip = hdr.ipv4.dst_addr; m.nm_pt = hdr.tcp.dst_port; m.nr_ip = hdr.ipv4.src_addr; m.nr_pt = hdr.tcp.src_port; }
    table e_fold {
        key = { hdr.tcp.src_port : exact; }
        actions = { e_fold_master; e_fold_out; }
        const default_action = e_fold_master();
        const entries = { (DNP3_L4_PORT) : e_fold_out(); }
        size = 2;
    }
    ...
    /* owner: exact direction-normalized 5-tuple. `cut` is the control-plane split
     * boundary (set_split(cut=28) for the 49 B target); this MVP is specialized to
     * cut=28 -> [28,21], so the carve below uses the matching constants. */
    action set_split(bit<8> cut) { m.owner = 8w1; m.policy = POL_SPLIT; }
    ...
    /* ===== THE SINGLE PREDICATE: do_shape = response-dir AND owner AND size==49,
     *       independent of DNP3 type. One table match sets one bit. ===== */
    action mark_shape()   { m.do_shape = 8w1; }
    action mark_noshape() { m.do_shape = 8w0; }
    table t_shape {
        key = { m.policy : exact; m.dir : exact; m.size49 : exact;
                m.eligible : exact; m.is_syn : exact; m.is_rst : exact; }
        actions = { mark_shape; mark_noshape; }
        const default_action = mark_noshape();
        const entries = { (POL_SPLIT, DIR_OUT, 8w1, 8w1, 8w0, 8w0) : mark_shape(); }
        size = 4;
    }
    ...
    apply {
        if (hdr.tcp.isValid()) {
            ...
            e_ctl.apply();
            t_policy.apply();
            t_shape.apply();               /* -> m.do_shape (the one gate) */

            m.rid = eg.egress_rid;

            if (m.rid == 16w0) {
                /* SOURCE (the deadline-release pass to dp9). do_shape -> split; else native. */
                if (m.do_shape == 8w1) {
                    eg_dprsr.mirror_type = MIRR_SHAPE;
                    m.mirror_session     = SHAPE_SESSION;
                    eg_dprsr.drop_ctl    = 3w1;      /* drop our own unicast; mcast copies carry it */
                }
            } else if (m.rid == 16w1) {
                /* window0 = frame[0..28) : header block + blk0 */
                hdr.blk1.setInvalid(); hdr.res3.setInvalid();
                hdr.ipv4.total_len = W0_TOTAL;
                hdr.tcp.flags = hdr.tcp.flags & CLR_PSH_FIN;
                m.translated = 1w1;
            } else if (m.rid == 16w2) {
                /* window1 = frame[28..49) (final segment) */
                hdr.dnp3_dl.setInvalid(); hdr.blk0.setInvalid();
                hdr.tcp.seq_no = hdr.tcp.seq_no + 32w28;
...
control EgDeparser(packet_out pkt, inout headers_t hdr, in eg_meta_t m,
    ...
    Checksum() ipv4_csum;
    Checksum() tcp_csum;
    Mirror()   shape_mirror;          /* no-arg ctor (typed ctor errors on TF1) */
    apply {
        if (eg_dprsr.mirror_type == MIRR_SHAPE) {
            shape_mirror.emit(m.mirror_session);
        }
        if (m.translated == 1w1) {
