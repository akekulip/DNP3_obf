/* DIAGNOSTIC PROBE (NOT the deliverable core). Minimal §4: the v5 bootstrap + ONE
 * ACK-deadline hold (~D3). Measures whether the SMALLEST timing addition fits on
 * v5. RESULT: 0 errors, 12/12 ingress stages (ZERO headroom), 7 registers,
 * tofino.bin. This is the floor; the full §4 lifecycle does NOT fit (see
 * full_integration_wall_probe.p4 + evidence/GATE2_INTEGRATION.md). Derived from the
 * frozen bootstrap_probe_v5.p4; not loaded, not run. */
/* ============================================================================
...
/* --- minimal D3 ACK-deadline (diagnostic): modular sign-bit compare, frozen idiom --- */
const bit<32> TICK_MASK  = 32w0xFFFFFF00;
const bit<32> ARMED_MARK = 32w0x00000001;
const bit<32> D_A_DEFAULT = 32w0x00010000;  /* control-plane tunable ACK hold offset (ticks) */
...
struct ig_meta_t {
    ...
    bit<32> ts32;         /* D3: ingress timestamp low 32b (unused scratch; now_word built inline) */
    bit<32> now_word;     /* D3: (ts & TICK_MASK) | ARMED_MARK */
    bit<32> d_a;          /* D3: ACK hold offset from params */
    bit<32> dl_cand;      /* D3: now_word + d_a, precomputed in MAU (SALU can't phv+phv) */
    bit<32> dl_age;       /* D3: now_word - deadline (sign bit => expired) */
    bit<8>  dl_expired;   /* D3: 1 = ACK deadline reached */
}
...
parser IgParser(packet_in pkt,
    ...
    state start {
        ...
        meta.ts32 = 32w0; meta.now_word = 32w0; meta.d_a = 32w0;
        meta.dl_cand = 32w0; meta.dl_age = 32w0; meta.dl_expired = 8w0;
...
control Ingress(inout headers_t hdr,
    ...
    /* ===== D3 ACK-deadline register (single-domain, like the others) ===== */
    Register<bit<32>, bit<1>>(1, 0) reg_deadline;
    RegisterAction<bit<32>, bit<1>, bit<32>>(reg_deadline) dl_arm = {
        void apply(inout bit<32> v, out bit<32> rv) { rv = meta.now_word - v; v = meta.dl_cand; }
    };
    ...
    /* D3: params (control-plane per-transaction D_A) */
    action set_da(bit<32> d) { meta.d_a = d; }
    table tbl_params_d3 { actions = { set_da; } default_action = set_da(D_A_DEFAULT); size = 1; }
    /* D3: build the deadline-aligned now word in TWO single-op tables (mask, then OR the
     * marker) — one action cannot do both (frozen D3 idiom; avoids action-spanning-stages). */
    ...
    action build_cand() { meta.dl_cand = meta.now_word + meta.d_a; }
    table tbl_build_cand { actions = { build_cand; } const default_action = build_cand(); size = 1; }
    /* D3: expiry via the sign-bit of age under the frozen mask */
    ...
    apply {
        if (meta.port_ok == 8w0) {
          ...
          if (meta.role == ROLE_TOKEN) {
            ...
            meta.dl_age = dl_read.execute(1w0);           /* D3: age of the ACK deadline */
            tbl_dl_expiry.apply();
            if (meta.dl_expired == 8w1) { to_fwd(); }     /* deadline reached -> release */
            else { to_ack_block(); }                      /* still held: re-enqueue behind reservoir */
            ...
            if (meta.role == ROLE_ACK) { meta.dl_age = dl_arm.execute(1w0); }  /* D3: arm T_A on native ACK */
