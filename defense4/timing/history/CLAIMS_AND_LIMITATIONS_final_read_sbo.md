> **Historical. Not the active claim authority.**
> This document states the claims and limitations of the retired `final_read_sbo`
> dataset: six captures taken in one session on 2026-08-13, with the size-shaping
> datapath active in both arms. Its numbers (999/488/599/499 samples, the 4.001 ms
> medians, MI 0.424 bits, balanced accuracy 0.592, five figures) do **not** describe the
> evidence the manuscript reports and must not be quoted as current.
>
> The active authority is `../CLAIMS_AND_LIMITATIONS.md`, which states the claims of
> `evidence/campaign_v1/`: 132 captures, 22 grouped runs, 63,360 exchanges, size shaping
> off in both arms. Kept for provenance only.
