/* ============================================================================
 * ►►►► defense4_timing_only.p4 — the timing mechanism, with the size layer removed ◄◄◄◄
 *
 * A candidate derived from the frozen evaluated program
 * `defense4_rrc_bor_unified12.p4` by deleting the release-replicate-CARVE size layer and
 * nothing else. The timing transaction is untouched: ADMIT -> HOLD -> RELEASE, with the
 * read lane anchored to the outstation's acknowledgment and the control lane to the
 * request, which is what the evaluation measures.
 *
 * What was removed, and it is only this:
 *   - the parser's 49 B eligibility gate and its four states (meta.payload49);
 *   - meta.shape_enable, the sixth knob on tbl_params, and meta.do_shape;
 *   - tbl_build_do_shape, the level-1 precompute that existed to keep the release
 *     gateway a single-field test;
 *   - the do_shape ternary column from tbl_decide_fresh and tbl_decide_deq, and the
 *     three entries that selected a SHAPE outcome;
 *   - cmt_shape, the RRC_TO_SHAPE macro, the four OUT_*_SHAPE outcome codes, and the
 *     multicast group they replicated into;
 *   - the whole egress pipeline: the DNP3 CRC-block headers, the block re-parse, the
 *     replication-id interpreter and the per-window checksum recomputation.
 *
 * Nothing else is touched. Every remaining table, register, queue and deadline is the
 * frozen program's. This is a COMPILE CANDIDATE: it answers what the size layer cost in
 * switch resources, and it has not been loaded or run.
 *
...
control Ingress(inout headers_t hdr,
    ...
    /* PROBE VARIANT 4, 2026-09-17: the write guard is the caller's job, not the SALU's, which
     * is how reg_bor_epoch already splits prepare/retire/read. That frees the comparison unit
     * the guard was spending, so the two the predicate needs both fit: armed (v != 0) and due
     * (v <= now_word). On the arming pass the deadline is in the future by construction, so
     * the arm action returns not-expired without comparing anything. */
    RegisterAction<bit<32>, bit<1>, bit<16>>(reg_bor_topj) topj_arm = {
        void apply(inout bit<32> v, out bit<16> rv) { v = meta.dl_val_topj; rv = 16w0; }
    };
    RegisterAction<bit<32>, bit<1>, bit<16>>(reg_bor_topj) topj_test = {
        void apply(inout bit<32> v, out bit<16> rv) {
            rv = 16w0;
            if (v != 32w0) { if (v <= meta.now_word) { rv = 16w1; } }
        }
    };
    ...
    DirectCounter<bit<64>>(CounterType_t.PACKETS) ctr_outcome;
    action cmt_drop()       { D3_DROP()       ctr_outcome.count(); }
    action cmt_fwd()        { D3_TO_FWD()     ctr_outcome.count(); }
    ...
    table tbl_decide_deq {
        ...
        const entries = {
            ...
            (ROLE_BLOCK, 8w1, V_BLOCK_PENDING, 8w0&&&8w0, 8w1, 8w0&&&8w0) : dec_o(OUT_RB_DL);
            (ROLE_BLOCK, 8w1, V_BLOCK_PENDING, 8w0&&&8w0, 8w0&&&8w0, 8w1) : dec_o(OUT_RB_TMO);
            (ROLE_BLOCK, 8w1, V_BLOCK_PENDING, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_loop(OUT_RB_LOOP);
            (ROLE_BLOCK, 8w1, V_BLOCK_LIVE, 8w0&&&8w0, 8w0&&&8w0, 8w1) : dec_o(OUT_RB_TMO);
            (ROLE_BLOCK, 8w1, V_BLOCK_LIVE, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_loop(OUT_RB_LOOP);
            (ROLE_BLOCK, 8w1, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_o(OUT_RB_STALE);
            /* ACK blocker (qid7, D4): LIVE/PENDING -> expired DL, budget TMO, else loop;
             * catch-all STALE. */
            (ROLE_BLOCK, 8w0, V_BLOCK_PENDING, 8w1, 8w0&&&8w0, 8w0&&&8w0) : dec_o(OUT_AB_DL);
            (ROLE_BLOCK, 8w0, V_BLOCK_PENDING, 8w0&&&8w0, 8w0&&&8w0, 8w1) : dec_o(OUT_AB_TMO);
            (ROLE_BLOCK, 8w0, V_BLOCK_PENDING, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_loop(OUT_AB_LOOP);
            (ROLE_BLOCK, 8w0, V_BLOCK_LIVE, 8w1, 8w0&&&8w0, 8w0&&&8w0) : dec_o(OUT_AB_DL);
            (ROLE_BLOCK, 8w0, V_BLOCK_LIVE, 8w0&&&8w0, 8w0&&&8w0, 8w1) : dec_o(OUT_AB_TMO);
            (ROLE_BLOCK, 8w0, V_BLOCK_LIVE, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_loop(OUT_AB_LOOP);
            (ROLE_BLOCK, 8w0, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_o(OUT_AB_STALE);
            /* ACK release (D4 always preserves) */
            (ROLE_ACK, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_o(OUT_ACK_RELEASE);
            /* RESPONSE release: attributed by T_RESP; shape iff armed+eligible */
            (ROLE_RESP, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0, 8w1, 8w0&&&8w0) : dec_o(OUT_REL_DL_FWD);
            (ROLE_RESP, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0, 8w0&&&8w0) : dec_o(OUT_REL_FO_FWD);
        }
        size = 32;
    }
    ...
    apply {
        if (meta.port_ok == 8w0) {
            ...
            meta.outcome = OUT_BADPORT;   /* -> cmt_drop at the single terminal tbl_commit */
        } else {
            ...
            if (meta.bor_pc == BPC_OPERATE || meta.bor_pc == BPC_TOKEN) {
                if (meta.dl_val_topj != DL_NO_WRITE) { meta.expired_topj = topj_arm.execute(0); }
                else                                  { meta.expired_topj = topj_test.execute(0); }
            }
#endif
