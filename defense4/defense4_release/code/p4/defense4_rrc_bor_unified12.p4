/* ============================================================================
 * ►►►► defense4_rrc_bor_unified12.p4 — CANONICAL Defense 4 program (one pipe) ◄◄◄◄
 *
 *   AUTHORITATIVE, one-program unified RRC + BOR design for ONE physical Tofino-1.
 *   Build flag: -DU_BOR (enables the Bounded-OPERATE-Release extension).
 *   Fit:        one ingress pipe, 12/12 MAU stages; egress <=6 stages (bf-p4c 9.13.1, 0 errors).
 *   Source sha256: 7ce30494...   Silicon binary sha: 33fa3a77.
 *   Silicon load:  LOADED and exercised on the real Tofino-1 against the physical SEL-751.
 *   Demonstrated on silicon (master-facing): RRC CLRT normalization (-> 4.001 ms std 0.02),
 *     fixed [28,21] segmentation (1280/1280 CRC/checksum-valid, 0 escapes), Formby CLRT
 *     feature-suppression, BOR ACK/echo anti-subtraction invariance (guarded OPERATE, all 32
 *     relay outputs OPEN).
 *   Claim boundary: relay-facing T0+J and exactly-once release were NOT directly observed
 *     (dp68 is internal, not a tap) -> inferred only. Single device -> signature REPLACEMENT,
 *     not multi-device indistinguishability. See evidence/E_FINAL/CLAIM_MATRIX.md and
 *     ../../../CLAIMS.md for the full grid.
 *   NOTE: the block below is the embedded RRC size-transform section header, kept verbatim;
 *     its "COMPILE PROBE ONLY" language predates the unified12 silicon campaign and is
 *     superseded by the status above. The BOR extension and its offline lifecycle/mutant/
 *     decision-table proofs are in ../verification and the archived RRC_BOR_UNIFIED12_*.md.
 * ----------------------------------------------------------------------------
 * ►►►► (embedded) defense4_rrc_kernel.p4 — RELEASE-REPLICATE-CARVE (RRC) native primitive ◄◄◄◄
 *
