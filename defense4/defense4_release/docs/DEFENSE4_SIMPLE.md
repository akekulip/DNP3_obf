...
![The whole idea in one line: traffic comes in, the switch decides what it is, applies a timing policy, steers it into hold queues, splits the answer at a clean cut point, and the master sees uniform traffic.](figs/FIG-0_overview_flow.png){width=98%}
...
- **Cut it at a clean line.** A DNP3 message has built-in checkpoints (CRC blocks). We choose to cut
  at one of those checkpoints — a simple, tidy place to split — so the pieces still add back up to the
  exact original. The 49-byte answer becomes two pieces: **28 bytes + 21 bytes**. No byte is changed,
  and the total is still 49 bytes; only the *shape* (how it's split) changes.
...
- **Shape:** every defended answer came out as the same two segments — **28 + 21 bytes** — 1,280 out of 1,280, with **none**
...
- **We can say:** the timing is normalized, the segment shape and packet count are uniform, the
  measured timing fingerprint is gone, no relay output ever actuated (all 32 stayed open), and it all
  fits on one switch.
...
> segment shape, same splitting, same timing, and control commands released on a hidden delay — so a
> watcher can no longer fingerprint the device on those features, and we did it without changing any
> of the relay's actual messages.
