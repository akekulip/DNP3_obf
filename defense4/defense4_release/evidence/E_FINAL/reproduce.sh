...
{ $RP $S/size_reconstruct.py $RAW/e1_native_size_shapeoff.pcap native
  $RP $S/size_reconstruct.py $RAW/e2_def_read.pcap defended   | grep -v '^pcap,'
  $RP $S/size_reconstruct.py $RAW/e2_def.pcap defended      | grep -v '^pcap,'
  for J in 2 6 12; do $RP $S/size_reconstruct.py $RAW/sbo_j$J.pcap defended | grep -v '^pcap,'; done
} > $C/size_verdict.csv
# --- stats + verdict JSON ---
