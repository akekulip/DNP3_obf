...
**Status: FEASIBILITY ANALYSIS — no implementation.** Per the plan (`acj_delay2.md`, PHASE 04) this
report must exist *before* implementation, and "do not proceed with implementation until the
feasibility report identifies a real enforcement mechanism." It does identify one; implementation
is **not** started and requires explicit human authorization. `next_phase_allowed = false`.
...
discrimination is *advisory only* (PSH is a hint). **`cls_bpf`/eBPF** parses variable IHL and TCP
data-offset and computes payload length exactly — the only classifier correct under options.
...
honors earliest-departure-time — this gives *independent* ACK vs response timing while forging
nothing); and `bpf_timer` for map-driven events. **XDP is the wrong hook** (ingress-only on the
mainline model; the need is egress release timing). **eBPF is required for the real mechanism.**
...
**Recommendation:** prototype **eBPF on tc `clsact` egress** (host-local for speed, re-hostable onto
a two-NIC bridge). It is the only candidate that satisfies all four requirements at once — exact
zero-payload classification, per-flow request-correlated state, and independent EDT-scheduled
release of *both* the real pure ACK and the real response — while forging nothing. Sequence:
(1) a `netem`+classifier smoke test to confirm that differentially delaying ACK vs response even
perturbs the fingerprint, then (2) the eBPF egress classifier + per-flow map + EDT release.
...
exists? Measured answer: no.** From `reports/ack_fingerprint_eval.md`: `ack_only` accuracy is
**0.810 before and 0.810 after** gap-normalization (chance 0.400). ACK mode is a **categorical**
feature (separate pure ACK present vs absent) invariant to gap magnitude — pinning SEL-751's
ACK→response gap to 20 ms does not make it *combined*.
...
**Yes.** eBPF on tc `clsact` egress with per-flow state and EDT (`skb->tstamp` + `fq`) is a real,
environment-supported mechanism that holds a *real* pure ACK and response to independently computed
departure times without forging or editing packets. It closes exactly the gap the application
cannot (holding an already-emitted ACK). Its decision logic ports to P4/Tofino; its scheduled
multi-ms release does **not** (Tofino cannot buffer for precise multi-ms delays) — so the eBPF
prototype does double duty: working mechanism *and* honest scoping of the eventual P4 port.
...
- Exact `TCP_QUICKACK` reset semantics on 5.15 (`tcp(7)`); writability of `__sk_buff->tstamp` in
  tc-egress and `fq` EDT enforcement behavior on 5.15.0-139 (field/module present; behavior untested
  here); `bpf_timer` helper *usable* (present in BTF; `bpftool` absent — probe via load test or
  install bpftool).
...
The feasibility report **identifies a real enforcement mechanism** (eBPF tc-egress EDT), answers
all nine plan questions, distinguishes the own-socket vs inline regimes, bounds the safety envelope,
and scopes the P4 port honestly. Per the plan, this is the gate to *consider* implementation — it is
**not** implementation, which the plan and the PI both require to start only on explicit
authorization.
