...
- **The implemented defense closes the ACK-*gap* magnitude but not the ACK *mode*.** Under `timing_gap_norm`, `timing`-family accuracy collapses (from 0.511 to 0.797) and clustering on timing loses the devices, but `ack_only` stays high (0.810 → 0.810) because a separate ACK still *exists* for the SEL-751 — normalizing its gap to a constant does not make it look combined.
