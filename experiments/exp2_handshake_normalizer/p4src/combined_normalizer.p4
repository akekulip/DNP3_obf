...
const bit<16> OUTSTATION_PORT = 20000;   /* ACK-mode: suppress this port's standalone pure ACK */
...
control Ingress(inout headers_t hdr, inout ig_meta_t md,
    ...
    apply {
        ...
        } else if (tcp && hdr.tcp.sport==OUTSTATION_PORT && md.pl==0
                   && (hdr.tcp.flags & 0x10)!=0 && (hdr.tcp.flags & 0x07)==0) {
            outc = 12;                                   /* ack_suppressed (ACK-mode collapse) */
            ig_dprsr.drop_ctl = 1;                       /* drop the standalone pure ACK */
