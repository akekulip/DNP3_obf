...
PROG="ackmode_pgen"; P=68; ACT="trigger_timer_one_shot"
iface=gc.ClientInterface("localhost:50052",client_id=96,device_id=0); iface.bind_pipeline_config(PROG)
...
def frame(sport,dport,flags,payload=b""):
    tcp=u16(sport)+u16(dport)+u32(100)+u32(1024)+bytes([5<<4,flags])+u16(8192)+u16(0)+u16(0)
...
FIX=[  # (name, frame, expected ctr index)
 ("outstation pure ACK (suppress)", frame(20000,44000,0x10), 1),
 ("outstation response PA+payload", frame(20000,44000,0x18,b"\x05\x64\x1f\x44"+b"\x00"*40), 0),
 ("master->outstation ACK (fwd)",   frame(44000,20000,0x10), 0),
]
def rd():
...
ac.entry_mod(tgt,[ac.make_key([gc.KeyTuple("app_id",0)])],[ac.make_data([gc.DataTuple("app_enable",bool_val=False)],ACT)])
print(f"ASIC ACK-MODE: {npass}/{len(FIX)}  (idx1=suppressed pure ACK, idx0=forwarded)")
