...
/* ==================== INTERNAL SEEDING (spike) ==========================
 * The K=64 blocker reservoir is minted INSIDE the chip by the READ itself:
 * the ingress deparser mirrors the READ into mirror session SEED_SESSION,
 * whose control-plane config points at multicast group MCG_SEED. That group
 * holds exactly 64 L1 nodes, each a one-port bitmap {PORT_L}, so the PRE
 * makes exactly 64 copies, all egressing the internal loopback dp8.
 * Egress rewrites each copy into a 0x88C1 blocker token carrying the READ's
 * own DNP3 application-control byte as the generation. No host frame, no
 * controller action per transaction. */
const MirrorType_t MIRROR_TYPE_I2E = 1;   /* ingress-to-egress mirror         */
...
/* mirror metadata prepended to each of the 64 seed copies: carries the READ's
 * generation across the ingress->egress boundary. One byte, so the mirror
...
struct headers_t {
    seed_md_h   seed;   /* egress only: valid on the 64 mirrored seed copies */
    ethernet_h  eth;
    ibspg_h     ib;
    ipv4_h      ipv4;
    tcp_h       tcp;
...
struct ig_meta_t {
    ...
    MirrorId_t mir_ses;    /* seed mirror session; bf-p4c forbids a constant here */
...
control Ingress(inout headers_t hdr,
    ...
    Counter<bit<64>, bit<1>>(1, CounterType_t.PACKETS) ctr_seed_mint;         /* READs that minted a reservoir  */
    Counter<bit<64>, bit<1>>(1, CounterType_t.PACKETS) ctr_seed_suppress;     /* retransmitted READs, not seeded */
        ...
        if (meta.port_ok == 8w0) {
            ...
            if (meta.dequeued == 8w0) {
                ...
                if (meta.role == ROLE_BLOCK) {
                    ...
                    to_fwd();
                    ctr_arm.count(0);
                    /* INTERNAL SEEDING: mint the K=64 reservoir from this READ.
                     * The forwarded READ itself is untouched (byte-identical);
                     * the 64 copies are made by the PRE off the mirror session.
                     *
                     * RETRANSMISSION GATE. meta.tag_ok == 1 means reg_tag ALREADY
                     * held this generation when this READ arrived, i.e. this is a
                     * TCP retransmission of a READ we have already seeded. Minting
                     * again would put 128 same-generation tokens in the reservoir,
                     * so only the first READ of a generation seeds. This is what
                     * keeps K bounded at exactly 64 under a data-plane trigger. */
                    if (meta.tag_ok == 8w0) {
...
control IgDeparser(packet_out pkt,
    ...
    /* the ONE new object in the ingress deparser: the seed mirror. It fires only
     * when the ingress control classified this frame as a DNP3 READ (ROLE_ARM).
     * The original packet is emitted below completely untouched. */
    Mirror() seed_mirror;
    apply {
...
parser EgParser(packet_in pkt,
    ...
    state start {
        ...
        /* dp8 is the internal loopback. Every packet the hold mechanism sends there
         * sets bypass_egress = 1 and therefore never reaches egress, so a packet
         * that arrives in EGRESS with egress_port == PORT_L can only be one of the
         * 64 mirrored seed copies. That is how the seed copies are distinguished
         * without adding bridged metadata to the byte-preserving forwarding path. */
    ...
    state parse_seed {
        pkt.extract(hdr.seed);      /* mirror metadata: the READ's generation */
        pkt.extract(hdr.eth);
        transition accept;
    }
    state parse_eth {
...
control Egress(inout headers_t hdr,
    ...
    Counter<bit<64>, bit<1>>(1, CounterType_t.PACKETS) ctr_seed_emit;

    /* turn one mirrored seed copy into one blocker token. Constant everywhere
     * except gen, which is the READ's own DNP3 application-control byte, carried
     * across the gress boundary in the mirror metadata. */
    action make_token() {
        hdr.eth.etype = ETHERTYPE_IBSPG_TOKEN;
        hdr.ib.setValid();
    ...
    apply {
        tbl_make_token.apply();
        if (hdr.ib.isValid()) { ctr_seed_emit.count(0); }
    }
}
control EgDeparser(packet_out pkt,
    ...
    apply {
        pkt.emit(hdr.eth);
        pkt.emit(hdr.ib);   /* valid ONLY on a seed copy; invalid on every
                             * forwarded frame, so the pass-through stays
                             * byte-identical. */
    }
}
