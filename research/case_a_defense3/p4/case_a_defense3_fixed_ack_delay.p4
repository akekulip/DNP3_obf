        ...
        if (meta.port_ok == 8w0) {
            ...
            if (meta.dequeued == 8w0) {
                ...
                if (meta.is_pktgen == 8w1) {
                    ...
                    /* E1: the marker delta, set HERE because tag_read_or_mark runs at
                     * level 2 and the decode table that would be the natural home for
                     * it runs at level 3. One-shot is enforced inside the SALU, not
                     * here, so a duplicate RESPONSE setting the same delta is safe. */
