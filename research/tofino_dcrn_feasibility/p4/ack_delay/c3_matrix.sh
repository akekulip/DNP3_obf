...
# Per mode it sets the switch mode + RE-APPLIES the dp8 loopback (ackA_setup resets it) + a ping gate,
...
# Cold-reload dcrn_ackA (clears ALL recirc state — held frames from prior txns) then re-setup the
# mode + dp8 loopback. Called before EACH interval so accumulation never exceeds one interval's txns.
reload_setup() {
  ssh "$SW" 'sudo pkill -x bf_switchd; sleep 1; sudo bash -c "nohup bash /home/decps/dcrn_m1/launch_ackA.sh >/home/decps/dcrn_m1/switchd_ackA.log 2>&1 </dev/null &"; sleep 18'
  ssh "$SW" "cd /home/decps/dcrn_m1; python3.8 ackA_setup.py --mode $SETMODE >/dev/null 2>&1; python3.8 dp8_loopback_ackA.py >/dev/null 2>&1; sleep 2; python3.8 ackA_read.py --reset >/dev/null 2>&1"
}
...
for RMS in "${RMS_ARR[@]}"; do
  ...
  for i in $(seq 1 "$N"); do
    ...
    ssh "$SW" "cd /home/decps/dcrn_m1; python3.8 ackA_read.py --reset >/dev/null 2>&1"
    ...
    ssh "$SW" "cd /home/decps/dcrn_m1; python3.8 ackA_read.py --json 2>/dev/null" > "$OUT/$LABEL.tel.json"
    ssh "$SW" "cd /home/decps/dcrn_m1; python3.8 ackA_read.py --reset >/dev/null 2>&1"
