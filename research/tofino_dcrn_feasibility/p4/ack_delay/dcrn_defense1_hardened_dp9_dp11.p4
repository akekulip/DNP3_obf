 ...
 *     its next pass, sees resp_seen==1, is directed to PORT_MASTER, and sets reg_ack_gone:=1 on
 *     that same pass. The response is directed to PORT_MASTER only on a pass where it reads
 *     reg_ack_gone==1 (+ a few guard passes).
 *
 * ZERO-INVERSION INVARIANT (the crux): a response frame is directed to PORT_MASTER ONLY on a pass
 *   where it reads reg_ack_gone==1; the ACK sets reg_ack_gone:=1 on the pass it is directed to
 *   PORT_MASTER. Register writes are visible only to strictly-later passes ⇒ the response's release
 *   pass is strictly later than the ACK's ⇒ on the SHARED FIFO egress queue of PORT_MASTER (both
...
const QueueId_t QID_HOLD    = 5;      // dp68 shaped hold queue. RELEASE paths leave qid at default 0
                                      // so ACK+response share ONE FIFO queue on PORT_MASTER (invariant).
    ...
    apply {
        ...
        if (!hdr.ethernet.isValid()) {
            ...
            if (hdr.bridge.role == ROLE_ACK) {
                ...
                if (ack_release == 1) {
                    ackgone_set.execute(meta.flow_id);     // set on the SAME pass we go to PORT_MASTER
                }
                if (ack_release == 1) {
                    ...
                    ig_tm_md.ucast_egress_port = PORT_MASTER;   // qid default 0 (shared FIFO); bridge stripped in egress
                } else {
                ...
                if (resp_release == 1) {
                    ...
                    ig_tm_md.ucast_egress_port = PORT_MASTER;   // qid 0 (shared FIFO, AFTER the ACK); bridge stripped in egress
                } else {
                    ig_tm_md.ucast_egress_port = PORT_RECIRC;
                    ig_tm_md.qid               = QID_HOLD;
                }
            ...
            if (meta.payload_len == 0) {
                ...
                if (qual == 1) {
                    ...
                    if (already == 0) {
                        ...
                        ig_tm_md.ucast_egress_port = PORT_MASTER;   // already holding -> duplicate, forward
                        events.count(EV_WATERMARK_BYPASS);
                    }
                } else {
                    ig_tm_md.ucast_egress_port = PORT_MASTER;   // not a qualified pure ACK -> forward
...
control DcrnEgress(
    ...
    apply {
        if (hdr.bridge.isValid()) {
            if (eg_intr_md.egress_port == PORT_MASTER) {
