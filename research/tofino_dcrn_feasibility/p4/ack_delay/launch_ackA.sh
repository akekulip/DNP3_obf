#!/usr/bin/env bash
#
# launch_ackA.sh — start bf_switchd with the Case-A (dcrn_ackA) program on the shared Tofino.
...
#   - /home/decps/dcrn_m1/dcrn_ackA.conf present (copied from this repo's dcrn_ackA.conf)
...
#
# After bf_switchd is up, drive the control plane from ackA_setup.py (separate process).
#
set -euo pipefail
...
tail -f /dev/null | "$SDE_INSTALL/bin/bf_switchd" \
    ...
    --conf-file /home/decps/dcrn_m1/dcrn_ackA.conf \
    --init-mode=cold \
    --status-port 7777
