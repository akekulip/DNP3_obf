/*******************************************************************************
 * dnp3_shadow_parser_hardened_dp9_dp11.p4 — PARSER-HARDENED dp9/dp11 VARIANT of FROZEN dnp3_shadow.p4.
 * NOT a replacement for the frozen file. Two changes vs frozen, both to fix the GATE-1 finding that the
 * parser dropped valid DNP3 link-only control frames (see PARSER_HARDENING_ROOTCAUSE_20260724.md):
 *   (A) parse_dnp3_dl descends to transport/app ONLY when link length >= 10 (full DNP3 app header present);
 *       a 0x0564 link-only/short frame passes through to accept (no extract-past-end).
 *   (B) the classifier labels a valid 0x0564 link-only frame LINK_OTHER (not MALFORMED).
 * Plus the measured-topology role binding: PORT_MASTER=dp9(dir0)=Vision, PORT_OUTSTATION=dp11(dir1)=Hulk.
 * No packet bytes/lengths/options/seq/checksums/DNP3 content are changed; forwarding stays byte-identical.
 *
 ...
 *
 * WHAT IT DOES:
 *   A transparent bump-in-the-wire between Vision (dp8, master) and Hulk (dp9, outstation).
 ...
 *
 * FORWARDING (shadow invariant): a frame that arrives on PORT_MASTER egresses PORT_OUTSTATION and vice
...
const PortId_t PORT_MASTER      = 9w9;    // master / dir 0 = Vision, MEASURED on dp9
const PortId_t PORT_OUTSTATION  = 9w11;   // outstation / dir 1 = Hulk, MEASURED on dp11
...
parser ShadowIngressParser(
    ...
    state parse_dnp3_dl {
        ...
        // PARSER HARDENING (A): descend to transport/app ONLY when the DNP3 link length field
        // guarantees a full application header worth of user data (length>=10 = 5 addr + >=5 user
        // bytes for dnp3_tp(1)+dnp3_app(4)). A valid 0x0564 link-only/short frame (length<=9) passes
        // through to accept -> forwarded unchanged, classified LINK_OTHER in the MAU (never dropped,
        // never MALFORMED). This fixes the extract-past-end drop of DNP3 link-only control frames.
        transition select(hdr.dnp3_dl.start_0, hdr.dnp3_dl.start_1, hdr.dnp3_dl.length) {
            (DNP3_START_0, DNP3_START_1, 8w10 .. 8w255) : parse_dnp3_tp;  // 0x0564 + full app header present
            (DNP3_START_0, DNP3_START_1, _)             : accept;          // 0x0564 link-only/short -> pass-through
...
control ShadowIngress(
    ...
    apply {
        ...
        if (ig_intr_md.ingress_port == PORT_MASTER) { meta.dir = 0; } else { meta.dir = 1; }
        ...
        else if (hdr.dnp3_dl.isValid() && hdr.dnp3_dl.start_0 == DNP3_START_0
                 && hdr.dnp3_dl.start_1 == DNP3_START_1) {
            // PARSER HARDENING (B): valid 0x0564 DNP3 link-only frame (no application user data) ->
            // pass-through, classified LINK_OTHER (NOT MALFORMED, NOT dropped).
            meta.classification = CLASS_LINK_OTHER; meta.notes = NOTE_OTHER_FC;
        }
        else {
        ...
        if (ig_intr_md.ingress_port == PORT_MASTER) {
            ig_tm_md.ucast_egress_port = PORT_OUTSTATION;   meta.eg_port = (bit<8>)PORT_OUTSTATION;
        } else {
            ig_tm_md.ucast_egress_port = PORT_MASTER; meta.eg_port = (bit<8>)PORT_MASTER;
        }
