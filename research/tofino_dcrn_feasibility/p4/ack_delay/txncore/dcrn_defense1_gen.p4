/*******************************************************************************
 * dcrn_defense1_gen.p4 — PHASE 2 generation-carrying variant of the FROZEN dcrn_defense1.p4.
 * NOT a replacement for the frozen file. Difference vs dcrn_defense1.p4: a per-flow `reg_gen`
 * register that is BUMPED on every arm and STAMPED into hdr.bridge.gen at hold-enter (replacing the
 * hardcoded `gen = 0`). This is the "generation carried" step and it COMPILES + FITS 12/12 ingress
 * on bf-p4c 9.13.1. The next step — ENFORCING freshness with a reg_gen read on the recirc path —
 * does NOT fit (measured: table-placement failure vs reg_armed/reg_expected_ack). See
 * COMPILE_FIT_RESULT.md. Enforcement therefore needs a compact redesign (human-gated, red-line #8).
 *
...
control DcrnIngress(
    ...
    Register<bit<8>,  bit<16>>(65536, 0) reg_gen;                // PHASE 2: per-flow generation (staleness guard)
    ...
    // reg_gen (PHASE 2 generation freshness): bump@arm (returns new gen to stamp), read@hold-enter and
    // @recirc (returns current gen). A recirc frame whose bridge.gen != current gen is a stale straggler.
    RegisterAction<bit<8>, bit<16>, bit<8>>(reg_gen) gen_bump = {
        void apply(inout bit<8> v, out bit<8> rv) { v = v + 1; rv = v; }
    };
    RegisterAction<bit<8>, bit<16>, bit<8>>(reg_gen) gen_read = {
    ...
    apply {
        ...
        if (!hdr.ethernet.isValid()) {
            ...
            if (meta.fc_ok == 1) {
                ...
                gen_bump.execute(meta.flow_id);            // PHASE 2: advance generation on every arm
            ...
            if (meta.payload_len == 0) {
                ...
                if (qual == 1) {
                    ...
                    bit<8> ackgen  = gen_read.execute(meta.flow_id);   // PHASE 2: read current gen (hoisted out of the header assign)
                    if (already == 0) {
                        hdr.bridge.setValid();
                ...
                bit<8> respgen = gen_read.execute(meta.flow_id);   // PHASE 2: read current gen (hoisted out of the header assign)
