#!/usr/bin/env bash
#
# launch_ackA.sh — start bf_switchd with the Case-A (dcrn_ackB) program on the shared Tofino.
...
#   - /home/decps/dcrn_m1/dcrn_ackB.conf present (copied from this repo's dcrn_ackB.conf)
...
tail -f /dev/null | "$SDE_INSTALL/bin/bf_switchd" \
    ...
    --conf-file /home/decps/dcrn_m1/dcrn_ackB.conf \
    --init-mode=cold \
    --status-port 7777
