...
Two modes (via --mode):
  ...
  case-a   — Case-A enforcement config: identical port/recirc/queue/seed setup PLUS
             ...
             COMPILED INTO dcrn_defense1.p4 (=4 passes); there is NO runtime target table for
...
Name provenance — ALL of the following were CONFIRMED against the compiled
  build_defense1_9.13.1/bfrt.json (local), not guessed:
...
Usage:
    python3.8 defense1_setup.py --mode forward           # C2 transparent forwarding
    python3.8 defense1_setup.py --mode case-a            # Case-A fixed-guard enforcement
    python3.8 defense1_setup.py --mode forward --dry-run # print plan only (works off-switch)
Author: Philip
"""
import sys
import argparse
...
# ── Case-A registers. Data field CONFIRMED = DcrnIngress.<reg>.f1 (build_defense1_9.13.1/bfrt.json) ──
...
def parse_args():
    ap = argparse.ArgumentParser(description="Case-A (dcrn_defense1) bring-up controller.")
    ap.add_argument("--mode", choices=["forward", "case-a"], default="forward",
                    help="forward = transparent (no arming, C2); case-a = fixed-guard enforcement")
...
def print_plan(args):
    ...
    print("=== defense1_setup PLAN (dry-run, no switch contact) ===")
...
def bring_up(args, gc):
    ...
    print("=== defense1_setup ===  program=%s  mode=%s" % (PROG, args.mode))
    ...
    # ── 2. recirculation on dp68 (NO pktgen — dcrn_defense1 synthesizes nothing) ──
    ...
    if args.mode == "case-a":
        ...
        print("  fc_allowlist: FC 0x%02x (READ) -> arm; all else bypass. GUARD_PASSES=4 compiled in." % FC_READ)
    else:
    ...
    print("ackA up (mode=%s). Capture on Vision: tcpdump -i enp59s0f0np0 'tcp port 20000'" % args.mode)
