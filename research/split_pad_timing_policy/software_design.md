...
~200 ms floor, but the number is provisional until measured. `size_decorrelate` is the *candidate*
default **conditioned on passing the self-leak test** `I(policy_choice; secret | class) ≈ 0`
(evaluation_plan §10.1 HO): a target that tracks N would re-encode N in the choice itself and must not
ship. Only the class **worst-case constant** target is provably self-leak-free without the test; until
HO passes on the rig, `constant` (class worst-case) is the safe default and `size_decorrelate`/`uniform`
are evaluation arms. [I/H — the P6 latency advantage over constant-time is a pre-registered test, not an
assumed result; and keeping P6 non-averageable requires the common target to exceed the worst native
time, which erodes part of that advantage — per gaps_and_novelty H3/H4]
