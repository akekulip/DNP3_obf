...
| **HM** (latency compounds) | CM3 cumulative added latency = L_hold + (n_chunks−1)·gap; NOT ≤ either alone | split+timing latency ≈ max, not sum | measured p95 completion vs model | paired bootstrap vs model prediction | primary (overhead) | model predicts within CI; any RTO overshoot = fail |
...
L_total(txn) = L_hold + L_pace
  ...
  L_pace  = (n_chunks − 1) · chunk_delay                  # split inter-chunk pacing
  n_chunks(bpc) = ⌈ n_crc_blocks / bpc ⌉                  # measured: 141→141/71/36/18 @bpc 1/2/4/8 [M]
HARD CONSTRAINT (cumulative, per fragment AND end-to-end):
  L_hold + L_pace  <  RTO_margin   (= κ · effective_RTO_measured_on_Vision, κ≈0.5)   [S/I]
```
**Key result (HM):** hold and pacing are **not independent** — both draw down the *same* RTO budget,
and for a multi-fragment response the pacing compounds per fragment. Aggressive split (bpc=1, 141
chunks) at a 10 ms inter-chunk delay alone = 1.4 s of pacing — **far past any RTO floor** → forces
early release (bypass) → privacy loss. The model therefore *bounds bpc from below* given the gap and
budget: `chunk_delay ≤ RTO_margin / (n_chunks−1)`.
...
subject to HARD CONSTRAINTS g(π) (feasible or infeasible, no trade-off):
  ...
  c6: cumulative added latency < RTO_margin  (per fragment AND end-to-end, §9.1)
...
= {cumulative added p95 < RTO_margin(measured Vision) AND correctness=100%}. Inherit F's pre-registered
**HE (P6 size-decorrelation beats P2 jitter at matched privacy)**; ADD **HM** (CM3 split+timing points
must respect the *cumulative* budget — split points with n_chunks too large fall outside the safe
...
|---|---|---|
| **Software replay server** | CM3 with **moderate bpc (4–8)** + P6 first-response decorrelation + P8 gap-norm; timing target = class worst-case | generating endpoint (no hold problem); moderate bpc keeps `L_pace`, bandwidth, and packet-count leak bounded while gap-norm covers the new gaps; the immediate, rig-validatable deliverable |
...
A fast and elitist multiobjective genetic algorithm: NSGA-II|K. Deb; A. Pratap; S. Agarwal; T. Meyarivan|2002|IEEE Transactions on Evolutionary Computation|10.1109/4235.996017|https://doi.org/10.1109/4235.996017|yes|peer-reviewed-journal|NA|NA|NA|NA|NA|multi-objective optimization (elitist non-dominated sorting + crowding distance)|sw|NA|algorithm|NA|NA|assumes cheap deterministic objectives; our objectives are noisy attacker estimates needing CI-aware dominance|core solver for the per-class combined-policy multi-objective search (§10)
An evolutionary many-objective optimization algorithm using reference-point-based nondominated sorting approach, Part I: solving problems with box constraints (NSGA-III)|K. Deb; H. Jain|2014|IEEE Transactions on Evolutionary Computation|10.1109/TEVC.2013.2281535|https://doi.org/10.1109/TEVC.2013.2281535|yes|peer-reviewed-journal|NA|NA|NA|NA|NA|reference-point many-objective optimization (≥4 objectives)|sw|NA|algorithm|NA|NA|scalability to reference-point placement; still assumes deterministic objectives|our program has ≥4 simultaneous objectives (many-objective) → NSGA-III over NSGA-II (§10.3)
Multiobjective evolutionary algorithms: a comparative case study and the strength Pareto approach|E. Zitzler; L. Thiele|1999|IEEE Transactions on Evolutionary Computation|10.1109/4235.797969|https://doi.org/10.1109/4235.797969|yes|peer-reviewed-journal|NA|NA|NA|NA|NA|hypervolume (S-metric) indicator + SPEA|sw|NA|methodology|NA|NA|hypervolume computation cost grows with objective count|Pareto-compliant scalar to compare platform/policy frontiers and against WF lower-bounds (§10.3)
pymoo: Multi-Objective Optimization in Python|J. Blank; K. Deb|2020|IEEE Access|10.1109/ACCESS.2020.2990567|https://doi.org/10.1109/ACCESS.2020.2990567|yes|peer-reviewed-journal|NA|NA|NA|NA|NA|MO optimization framework (NSGA-II/III, constraints, HV, decision making)|sw|NA|software-tool|NA|NA|implementation/tooling, not a new method|the toolkit implementing NSGA-II/III + constrained domination + hypervolume for §10
On a bicriterion formulation of the problems of integrated system identification and system optimization (epsilon-constraint method)|Y. Y. Haimes; L. S. Lasdon; D. A. Wismer|1971|IEEE Transactions on Systems, Man, and Cybernetics|10.1109/TSMC.1971.4308298|https://doi.org/10.1109/TSMC.1971.4308298|yes|peer-reviewed-journal|NA|NA|NA|NA|NA|epsilon-constraint scalarization for Pareto frontier generation|sw|NA|methodology|NA|NA|generates weakly-Pareto points; needs a solver per epsilon slice|principled name for the per-platform knob sweep → deterministic privacy-latency/-bandwidth/-hardware frontiers (§10.3, §11)
