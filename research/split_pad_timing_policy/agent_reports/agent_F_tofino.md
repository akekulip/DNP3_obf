...
CROB count, leaks on the **size** channel (14.6 B/CROB, R²=0.9999 [M]) even more cleanly than on
timing (0.179–0.214 ms/CROB [M]). Nothing Tofino can do in-phase touches size — split preserves
...
_None. This design reuses only works already in the 102-paper matrix and Agent E's hardware rows
(Open-Tofino, RMT/Bosshart, Wu recirc, DPTP, NetVRM, TEA, Nimble, NetWarden, SP-PIFO/PIFO/PIEO,
P4-TAS, Corundum, 802.1Qbv, IEEE 1588, calendar queues, BlueField / Accurate Send Scheduling / DOCA),
plus the lab's internal Tofino constraint doc and measured rig evidence. No new external work is cited,
so no new matrix rows or BibTeX are added (integrity: do not invent citations)._
