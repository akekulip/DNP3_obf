/* ============================================================================
 * p12_combined.p4 — THE CAMPAIGN QUESTION: does the full target architecture fit,
 *                   and at what measured ingress stage cost?
 ...
 *       chunk set selected by select(tcp.data_offset, ipv4.total_len) IN THE
 *       TCP-EXTRACT STATE, so the residual is empty and pad64..pad1 land after the
 *       complete inner datagram. Carried over VERBATIM — same 13 classes, same one
 *       fixed 128 B target. Not one entry added, so the egress column stays
 *       comparable with the standalone P6c measurement.
 *
 * ------------------------------------------------------------------------------
...
/* ========================= EGRESS — piece 3, verbatim ===================
 * Fully consume ip + tcp + payload so the deparser residual is empty. Any frame that
 * is not ihl=5 / data_offset=5 / a known total_len falls through to `accept` with the
...
parser EgParser(packet_in pkt, out eg_headers_t hdr, out eg_meta_t meta,
     ...
     * ipv4.total_len in a state placed AFTER the tcp extract is a known bf-p4c
     * 9.13.x failure mode on this toolchain. */
    state parse_tcp {
        pkt.extract(hdr.tcp);
    ...
    /* Payload chunks extracted DESCENDING; the deparser emits DESCENDING too, so
     * every subset reconstructs the payload byte-identically. */
...
control Egress(inout eg_headers_t hdr, inout eg_meta_t meta,
    ...
    /* Keyed on the MEASURED egress frame length. Only frames whose payload the parser
     * fully consumed can match, because the 13 lengths correspond exactly to the 13
     * parsed classes; anything else takes the default and is unchanged. */
    table size_norm {
        ...
        /* KEYS CORRECTED 2026-07-25 after silicon measurement: eg_intr_md.pkt_length
         * reports ipv4.total_len + 18, i.e. the Ethernet frame length PLUS the 4-byte
         * FCS. The original keys were total_len + 14 and were therefore short by 4, so
         * every frame missed and fell through to pad_none. Determined empirically: of
         * the 13 parser classes only total_len=48 (pkt_length 66) hit a key, and only
         * that class normalized on silicon. The pad amounts are UNCHANGED and remain
         * correct -- pad = 128 - wire = 128 - (pkt_length - 4) -- it is only the keys
         * that move. Target is 128 B ON THE WIRE, which is what an observer measures. */
        const entries = {
            16w64  : pad_d68();  16w66  : pad_d66();  16w70  : pad_d62();
            16w78  : pad_d54();  16w80  : pad_d52();  16w92  : pad_d40();
            16w93  : pad_d39();  16w95  : pad_d37();  16w105 : pad_d27();
            16w107 : pad_d25();  16w112 : pad_d20();  16w119 : pad_d13();
            16w124 : pad_d8();
        }
