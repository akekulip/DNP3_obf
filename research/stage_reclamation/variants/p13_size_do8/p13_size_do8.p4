...
parser EgParser(packet_in pkt, out eg_headers_t hdr, out eg_meta_t meta,
    ...
    state parse_tcp {
        ...
        transition select(hdr.ipv4.total_len) {
            16w40  : pl_0;     /* <- replay corpus: pure ACK,   do=5   (see note) */
            16w46  : pl_6;
            16w48  : pl_8;
            16w52  : pl_12;    /* <- measured corpus: pure ACK, do=8   (906 frames) */
            16w60  : pl_20;    /* <- measured corpus: SYN,      do=10  (  2 frames) */
            16w62  : pl_22;
            16w74  : pl_34;    /* <- measured corpus: REQUEST,  do=8   (198 frames) */
            16w75  : pl_35;
            16w77  : pl_37;
            16w87  : pl_47;    /* <- measured corpus: RESPONSE, do=8   (400 frames) */
            16w89  : pl_49;    /* <- measured corpus: RESPONSE, do=8   (400 frames) */
            16w94  : pl_54;
            16w101 : pl_61;
            16w106 : pl_66;    /* <- measured corpus: RESPONSE, do=8   (198 frames) */
    ...
    /* pl_0 consumes nothing: at total_len = 40 the IP datagram ends with the TCP base
     * header, so there is nothing past it to consume and the residual holds only the
     * sending MAC's Ethernet padding — which is outside the datagram, so the pads are
     * still a valid trailer. Only the OUTPUT SIZE depends on that padding being the
     * standard 6 bytes; safety does not. */
...
control Egress(inout eg_headers_t hdr, inout eg_meta_t meta,
    ...
    /* ===================== THE SILICON FIX ==================================
     * P6c/P12 keyed this table on `eg_intr_md.pkt_length`, and on real silicon it
     * NEVER FIRED: a 30-transaction campaign moved ctr_size_failopen by ~113 and
     * ctr_size_normalized by ZERO, including for replay frames deliberately built at
     * data_offset = 5 with total_len = 94 / wire 108, which had both a matching
     * parser class (pl_54) and a matching table entry (16w108).
     *
     * MEASURED ON SILICON 2026-07-25 (nine crafted probe frames, one injected at a
     * time; exactly one — total_len = 48 — normalized, which uniquely identifies the
     * convention): `eg_intr_md.pkt_length` is the WIRE LENGTH PLUS 4, i.e. it counts
     * the FCS. The SDE header documents the field only as "Packet length, in bytes"
     * (tofino1_base.p4:280) and fixes no offset convention, so every P6c/P12 entry
     * was short by exactly 4 and missed.
     *
     * The fix is to STOP DEPENDING ON IT — the +4 is an ACCOUNTING convention only,
     * and reading it back correctly is a second thing to get wrong. Note also that
     * "+18 above total_len" holds only while wire == 14 + total_len; for a frame the
     * sending MAC padded up to the 60 B Ethernet minimum the field must track the
     * padded bytes, so the durable form of the rule is `pkt_length = wire + 4`.
     *
     * The pad amount does not need a measured total, because on this traffic the wire
     * length is a DETERMINISTIC function of the declared IP length:
     *
     *     wire = max(60, 14 + total_len)          <- 60 = the Ethernet minimum
     *     pad  = 128 - wire
     *
     * TARGET CONVENTION, stated so no reader has to guess: 128 B ON THE WIRE, FCS
     * EXCLUDED — the number a capture reports and an observer measures. The same
     * frame reads as `pkt_length` = 132. The FCS is NOT in the deparser residual:
     * on silicon, a 108 B response took pl_54 (payload fully consumed, residual
     * should be 0) and was forwarded at exactly 108 B, which it could not have been
     * had 4 FCS bytes been sitting in the residual. So emitting P pad bytes yields
     * wire_out = wire_in + P, with no FCS correction anywhere in the arithmetic.
     *
     * and that is exact for the whole corpus: measured over all 2104 packets of
     * `Traffic Trace/SEL751.pcap`, frame.len - ip.len == 14 for EVERY packet, with
     * no Ethernet trailer and no frame below 60 B. The one class that relies on the
     * `max(60, ...)` arm is total_len = 40 (a data_offset = 5 pure ACK, 54 B before
     * the sending MAC pads it to 60); it is marked below.
     *
     * KEY, and why each field is there:
     *   tcp.isValid()  proves the egress parser REACHED parse_tcp, i.e. the frame is
     *                  IPv4 / ihl = 5 / TCP. Without it an ihl != 5 frame would carry
     *                  a matching total_len while its options+TCP+payload sat in the
     *                  deparser residual, and the pads would be emitted BEFORE that
     *                  residual — splitting the IP datagram in two. P6c and P12 both
     *                  had this hole; it is closed here.
     *   total_len      proves the parser CONSUMED the datagram past the TCP base
     *                  header. The values below are a subset of the values the egress
     *                  parser selects on, so tcp-valid AND a listed total_len together
     *                  imply the frame took its pl_* state and the residual is empty.
     *
     * Anything else — an unknown length, ihl != 5, non-TCP, non-IPv4, or an OVERSIZE
     * frame (total_len > 114) — has no entry and takes `pad_none`: forwarded
     * byte-for-byte unchanged, never truncated. */
    table size_norm {
        ...
        const entries = {
            /* valid, total_len         pad      wire  out   */
            (true, 16w40 ) : pad_d68();  /*  60*  128  * MAC-padded from 54; see above */
            (true, 16w46 ) : pad_d68();  /*  60   128  */
            (true, 16w48 ) : pad_d66();  /*  62   128  */
            (true, 16w52 ) : pad_d62();  /*  66   128   measured corpus: pure ACK  do=8  */
            (true, 16w60 ) : pad_d54();  /*  74   128   measured corpus: SYN       do=10 */
            (true, 16w62 ) : pad_d52();  /*  76   128  */
            (true, 16w74 ) : pad_d40();  /*  88   128   measured corpus: REQUEST   do=8  */
            (true, 16w75 ) : pad_d39();  /*  89   128  */
            (true, 16w77 ) : pad_d37();  /*  91   128  */
            (true, 16w87 ) : pad_d27();  /* 101   128   measured corpus: RESPONSE  do=8  */
            (true, 16w89 ) : pad_d25();  /* 103   128   measured corpus: RESPONSE  do=8  */
            (true, 16w94 ) : pad_d20();  /* 108   128   replay corpus:   RESPONSE  do=5  */
            (true, 16w101) : pad_d13();  /* 115   128  */
            (true, 16w106) : pad_d8();   /* 120   128   measured corpus: RESPONSE  do=8  */
        }
        const default_action = pad_none();
        size = 16;
    }

#ifdef SIZE_LEN_PROBE
    /* ---- DIAGNOSTIC, compiled in only with -DSIZE_LEN_PROBE ------------------
     * Answers, in the same run that carries the fix, the question the silicon run
     * raised: what does `eg_intr_md.pkt_length` actually report on this target, and
     * how far does the egress parser get? Registers, not Counters, so they read live
     * without operations_execute("SyncCounters"). None of this is on the forwarding
     * path and none of it is needed by the fix — the fix no longer reads pkt_length. */
    Register<bit<32>, bit<9>>(512, 0) reg_eg_pktlen;  /* histogram of pkt_length      */
    Register<bit<32>, bit<9>>(512, 0) reg_eg_totlen;  /* histogram of ipv4.total_len  */
    Register<bit<32>, bit<1>>(1,   0) reg_eg_ipv4_ok; /* frames with IPv4 valid       */
    Register<bit<32>, bit<1>>(1,   0) reg_eg_tcp_ok;  /* frames that reached parse_tcp*/

    RegisterAction<bit<32>, bit<9>, bit<32>>(reg_eg_pktlen) ra_pktlen = {
        void apply(inout bit<32> v) { v = v + 1; } };
    RegisterAction<bit<32>, bit<9>, bit<32>>(reg_eg_totlen) ra_totlen = {
        void apply(inout bit<32> v) { v = v + 1; } };
    RegisterAction<bit<32>, bit<1>, bit<32>>(reg_eg_ipv4_ok) ra_ipv4_ok = {
        void apply(inout bit<32> v) { v = v + 1; } };
    RegisterAction<bit<32>, bit<1>, bit<32>>(reg_eg_tcp_ok) ra_tcp_ok = {
        void apply(inout bit<32> v) { v = v + 1; } };
#endif
...
#ifdef SIZE_LEN_PROBE
        ra_pktlen.execute(eg_intr_md.pkt_length[8:0]);
        if (hdr.ipv4.isValid()) { ra_ipv4_ok.execute(0); ra_totlen.execute(hdr.ipv4.total_len[8:0]); }
        if (hdr.tcp.isValid())  { ra_tcp_ok.execute(0); }
#endif
    }
}
