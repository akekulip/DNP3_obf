/* ============================================================================
 * p7_parser_classify.p4 — variant P7: PARSER-PRODUCED CLASSIFY METADATA
 *
 * Part 12 `ibspg_hold_response.p4` (P0) with ONE architectural variable changed:
 * `meta.dequeued` and `meta.budget_zero` are produced in the INGRESS PARSER
 * instead of in MAU stage 0. Nothing else differs — same registers, same state
 * machine, same tables, same actions, same wire format.
 *
 * WHY THIS SAVES A STAGE. In P0 the ARM write-driver table is pinned to stage 1
 * because its gateway reads `meta.dequeued`, which a stage-0 table produces — a
 * hard producer/consumer edge at the head of the serial chain. Deleting the
 * producer removes the edge, the driver table relaxes into stage 0, and every
 * downstream link shifts up one.
 *
 * THE PARSER RULE THAT MATTERS. Parser metadata may be written only ONCE per
 * path: initialising a field to 0 in `start` and then overwriting it on a branch
 * is a hard error, because the parser has no clear-on-write. Both fields
 * therefore drop their zero initialiser and rely on the compiler's all-zero
 * default (`init_zero:` in the .bfa), with only the 1-valued branch assigning.
 *
 * `budget_zero` needs a 32-bit parser select on hdr.ib.seq, which is the one
 * unknown here — the parser match resources are narrow, so this is exactly the
 * kind of construct that can be rejected. Measured result: see compile_note.md.
 * ==========================================================================*/
#include <core.p4>
#include <tna.p4>
...
parser IgParser(packet_in pkt,
    ...
    /* P7: `dequeued` and `budget_zero` are produced HERE, not in MAU stage 0.
     * Parser metadata may be written only ONCE per path — assigning a default in
     * `start` and then overwriting it on a branch is a hard error ("no clear-on-write").
     * So the zero initialisers for these two fields are deleted and the all-zero
     * default the compiler emits (`init_zero:` in the .bfa) is relied on instead;
     * only the 1-valued branch assigns. */
    state start {
...
control Ingress(inout headers_t hdr,
    ...
    apply {
        if (!hdr.ib.isValid()) {
            ...
            /* ---------- classify ----------
             * P7: `dequeued` and `budget_zero` now arrive from the parser, so the
             * stage-0 producer table and, with it, the stage-0 -> stage-1 edge that
             * pinned the ARM write-driver table are both gone. */
