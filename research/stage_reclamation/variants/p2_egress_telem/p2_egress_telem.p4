/* ============================================================================
 * p2_egress_telem.p4 — variant P2 (WS3): EGRESS TELEMETRY OFFLOAD
 *
 * Part 12 `ibspg_hold_response.p4` (P0) with ONE architectural variable changed:
 * the RELEASE-SIDE evidence functions move from ingress to egress AS A GROUP.
 * The state machine, the queue assignment, the deadline decision and every
 * hold/release decision stay exactly where they were.
 *
 * MOVED (non-state-changing evidence only):
 *   reg_ts_ack_arm             t_ack, the left edge of the measured interval
 *   reg_ts_first_resp_release  the right edge — the release timestamp
 *   ctr_ack_arm, ctr_ack_bypass    release-role counters for the ACK
 *   ctr_resp_release               released-packet count
 *
 * NOT MOVED, and why — these are not a matter of taste:
 *   reg_ts_first_block, ctr_block_enq, ctr_resp_enq : the fresh BLOCK and the
 *     enqueued RESP are sent with bypass_egress=1, so they never reach egress.
 *   reg_ts_block_term, ctr_block_term_*             : a terminating blocker is
 *     DROPPED in ingress; it has no egress at all.
 *   ctr_arm, ctr_nonibspg                           : likewise dropped.
 *   Routing any of those through egress would add an egress traversal to every
 *   recirculation pass, i.e. it would change the very timing under test. The
 *   brief forbids moving anything that determines whether a packet is held or
 *   released, and this is that.
 *
 * HOW THE ACK's QUALIFICATION REACHES EGRESS. A released RESP is
 * self-identifying in egress (an enqueued RESP bypasses egress, so a RESP seen
 * there can only be a released one), but "did this ACK qualify" cannot be
 * recomputed without state. A 5-byte BRIDGE header carries the qualification bit
 * and the INGRESS timestamp across. Carrying the ingress timestamp — rather than
 * stamping in egress — keeps the recorded evidence bit-identical to P0's: the
 * offload changes WHERE the register lives, not WHAT it records. Stamping in
 * egress instead would shift both edges by one pipeline traversal and would put
 * `release_tail = ts_first_resp_release - ts_block_term` on two different clocks.
 *
 * BYTE PRESERVATION IS UNAFFECTED. The bridge header is made valid ONLY inside
 * to_host(), i.e. only on packets with bypass_egress=0, and the egress deparser
 * never emits it. Blocker tokens and the held response are enqueued with
 * bypass_egress=1 and no bridge header, so what is queued and what leaves the
 * switch are the same bytes as in P0.
 *
 * EXPECTED STAGE RESULT: ZERO ingress stages saved. WS1 forensics measured that
 * DELETING the entire timestamp bank outright leaves P0 at 12 stages; deletion
 * upper-bounds any offload, so the bound is zero. This variant is compiled to
 * confirm that bound and to quantify what it DOES buy: ingress SRAM, map RAM,
 * SALU and Stats-ALU headroom for the DNP3 work that has to fit alongside.
 * ==========================================================================*/
#include <core.p4>
#include <tna.p4>
...
struct ig_meta_t {
    ...
    /* ev_ack_arm / ev_resp_release are gone: those two events are now evidenced
     * in egress, where the packet itself identifies the event. */
}
...
control Ingress(inout headers_t hdr,
    ...
    /* immediate ACK, or released RESP -> dp9. The bridge header is made valid HERE
     * and nowhere else, so it can only ride packets that actually enter egress. */
        ...
        if (!hdr.ib.isValid()) {
            ...
            if (meta.dequeued == 8w0) {
                ...
                if (hdr.ib.role == ROLE_BLOCK) {
                     ...
                     * Egress counts it and stamps the release time — a RESP in egress
                     * can only be a released one, so no bridge flag is needed. */
                    to_host(8w0);
                } else {
...
control Egress(inout headers_t hdr,
    ...
    /* The moved evidence. Same write-if-zero shape as in P0, and stamping the
     * INGRESS timestamp carried on the bridge, so the recorded values are
     * bit-identical to what P0 recorded in ingress. */
    ...
    Counter<bit<64>, bit<1>>(1, CounterType_t.PACKETS) ctr_ack_bypass;   /* non-qualifying  */
    Counter<bit<64>, bit<1>>(1, CounterType_t.PACKETS) ctr_resp_release; /* released RESPs  */
