...
To evaluate in-network obfuscation on production-representative hardware, we implemented Defense~4
...
generator that seeds strict-priority blocker reservoirs; (iv)~the RRC release, PRE replication, and
egress carving path; (v)~the BOR epoch-and-hold state machine; and (vi)~two independent
Traffic-Manager scheduling domains. A single terminal table applies every port, queue, drop, bypass,
and multicast effect. The program places in one ingress pipe within the twelve-stage match-action
budget.
...
to the switch and carry no external cable: \code{dp8} recirculates responses for the RRC ladder,
\code{dp10} holds the OPERATE for BOR, and \code{dp68} injects generated tokens and clones. The
...
holds the RRC reservoirs on \code{dp8} (queues \code{qid7}--\code{qid4}) and the BOR queue pair on
...
This structure keeps the disposition auditable from a single table and is what lets RRC and BOR share
...
the BOR timing argument: because the ACK exists only after the held OPERATE is released, an
...
\subsection{RRC: release, PRE replication, and egress carving}
RRC (Release-Replicate-Carve) normalizes the segment shape and the ACK-to-response timing of an
...
\subsection{BOR: epoch, hold, release, and retirement}
BOR (Block OPERATE, then Release) governs the one control command. \textbf{Preparing an epoch.} A
...
\textbf{Why two domains.} RRC seeds ACK and response reservoirs on high-priority queues. On one shared
...
RRC ladder on \code{dp8} and the BOR queue pair on \code{dp10}, so each mechanism arbitrates on its own
...
the session and timing parameters and the $J$ codebook, and initializes the BOR registers. Every write
...
The BOR timing argument holds only on a flow without TCP timestamps; the pipeline gates on the
...
The program compiles for one Tofino-1 ingress pipe within the twelve-stage budget; the BOR state uses
four registers in a domain gated so that an OPERATE token never reaches the RRC registers. The
