...
The standard answer to such leakage is traffic obfuscation, and it is well developed for general
internet traffic. The goal is to make the observed traffic independent of the real traffic: padding
hides a flow's size by adding bytes; morphing transforms one class's size distribution into
another's~\cite{wright2009}; adaptive padding inserts dummy packets to break the inter-packet timing
pattern~\cite{juarez2016}; and constant-rate and burst-molding schemes force a fixed rate or a
colliding shape at a bounded cost~\cite{tamaraw2014,walkietalkie2017}. The state of the art moves this
into the network so that end hosts need not cooperate. Ditto is the clearest example: running on a
programmable switch, it pads every packet and injects chaff packets so that the size, timing, and
volume the switch emits follow a fixed template that is independent of the payload, at line rate and
without any change to the end hosts~\cite{ditto2022}, and related systems obfuscate flows against
traffic analysis in the data plane in the same spirit~\cite{securitas2026,minos2025,nethide2018}.

This approach does not fit ICS traffic such as DNP3, for a reason that is structural rather than
incidental. General obfuscation is free to \emph{add and fabricate} bytes and packets, and it targets
an \emph{opaque} flow whose only observable is a coarse volume or size pattern. DNP3 violates both
