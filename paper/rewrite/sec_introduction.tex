...
open a breaker or forge a measurement, it first learns which devices are on the network and how they
behave. In December 2015, remote intruders mapped a Ukrainian distribution utility, then used that
...
single 49-byte segment and a CLRT median of 1.3~ms, while a control select answers at 2.1~ms; the two
transaction classes are separable by timing alone, and prior traffic-analysis work shows that such
size-and-timing side channels recover content even on encrypted sessions~\cite{sirinam2018}.

Normalizing these features on a deployed DNP3 link is constrained on both sides. \emph{First},
...
shape of DNP3 traffic in the network, at line rate, while preserving protocol validity and requiring
no endpoint change. We address this gap with \emph{Defense~4}, an in-network obfuscation layer that
runs entirely in the data plane of one Intel Tofino-1 switch placed at the outstation edge. Defense~4
holds each response in strict-priority switch queues and releases its ACK and response at fixed policy
offsets from the request arrival, so the master-visible CLRT becomes a constant. It then replicates
the eligible 49-byte response and carves it at an existing DNP3 CRC-block boundary into a fixed
$[28,21]$ segment vector, so no DNP3 byte is modified and the master reassembles the identical
response. We call this timing-and-segmentation primitive RRC (Release-Replicate-Carve).

A control command needs a second primitive. The Select-before-Operate exchange exposes a physical
operation-timing fingerprint: the delay from an OPERATE to its confirmation reflects the relay's
actuation. RRC alone cannot hide this delay, because releasing the OPERATE on a fixed schedule would
still reveal it. Defense~4 adds BOR (Block OPERATE, then Release), which holds the OPERATE and is
designed to release it once at a secret internal delay while anchoring the master-visible ACK and echo
to the original arrival time, so the observable gap does not vary with the hidden delay. RRC and BOR
cannot share one strict-priority queue ladder, because the RRC reservoirs starve the OPERATE queues
after the command arrives; Defense~4 therefore uses two independent switch scheduling domains, both
internal to the same Tofino and the same program.
...
release delays; and a classifier that separated read from select by CLRT with 0.59 balanced accuracy
...
The evaluation is bounded. We test one outstation, so Defense~4 replaces one device's CLRT and
...
\begin{itemize}
\item \textbf{A hardware timing primitive for absolute-offset response shaping.} RRC holds the ACK and
response in strict-priority reservoirs and releases them at fixed offsets from the request timestamp,
normalizing the master-visible CLRT to a constant.
...
\item \textbf{A control-command timing primitive.} BOR holds the OPERATE and anchors the master-visible
ACK and echo to the request arrival, and we identify why RRC and BOR require two independent switch
scheduling domains. To the best of our knowledge, this is the first in-network defense to normalize
both the response timing and the segment shape of a live industrial-control protocol while preserving
its bytes and CRCs.
\item \textbf{A one-program implementation and a bounded hardware evaluation.} We implement Defense~4
