...
arrival time, so the observable gap does not vary with the hidden delay. The two mechanisms cannot
share one strict-priority queue ladder, because the response-shaping reservoirs starve the OPERATE
queues after the command arrives; the defense therefore uses two independent switch scheduling domains,
both internal to the same Tofino and the same program.
...
two independent switch scheduling domains. To the best of our knowledge, this is the first in-network
defense to normalize
