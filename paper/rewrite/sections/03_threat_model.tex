...
outstation's own processing as Section~\ref{sec:bg} describes.  On our relay the first interval's median differs by 0.8~ms between a poll and a control, against a request-to-acknowledgment interval that differs by
...
\textbf{Two adversaries, not one.} We evaluate two conditions, which answer different
questions, and we report both. The \emph{fixed} adversary learns transaction classes from undefended traffic and applies that
...
The \emph{adaptive} adversary collects obfuscated traffic and retrains on it. It assumes more
access than the reconnaissance setting describes, and it answers the question of what remains
learnable once the defense is known: it recovers 0.651 balanced accuracy where the fixed one is
left at chance. Both conditions are ours, chosen to bracket the problem. The second is the one
...
so RO3 concerns the timing channel and not the command semantics. It forwards every packet at its original size, so it adds no size or segmentation signature of its
own; the delays it imposes are themselves observable, and an adversary who can time packets can
see that something is holding them. And the relay-facing link is not instrumented, so the per-transaction hold, drawn from a
