...
\caption{How one pipeline realizes the schedule. Ingress classifies each transaction and computes
  ...
  deadline expires. The four-queue ladder holds what the outstation returns, for READ and for both
  phases of a control; the second pair holds the outgoing OPERATE on its way to the relay, so it
  is not a second reply lane. Solid lines are master-facing and observed; dashed lines are not, as
  in Figure~\ref{fig:observation}.}
\label{fig:design}
\end{figure}
