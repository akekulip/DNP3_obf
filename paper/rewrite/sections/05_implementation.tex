...
transaction has retired are all forwarded unchanged, so such an exchange loses its protection but
not its delivery. Every hold is bounded by a budget on blocker passes: once a reservoir has spent
...
packet-generator configuration, writes the timing parameters and reads them back, and refuses
configurations the data plane could not honor: the admissible budget of
Section~\ref{sec:design:params} and, on the control lane, that $A$ exceeds the largest admissible
$J$ plus a bound on the outstation's acknowledgment latency. The outstation is an SEL-751A feeder
