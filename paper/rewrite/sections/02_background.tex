...
SELECT (function code 3) that names an output point, and the outstation responds by echoing the
request; the master then sends an OPERATE (function code 4), which the outstation confirms with
...
CLRT of a READ is 1.272~ms and that of a SELECT is 2.107~ms, both with long tails, so the two
transaction classes are already partly separable from timing alone (Section~\ref{sec:eval}).
