...
already in the past when the response appears, so the switch marks it late and commits it to the
response-hold queue, from which it leaves when that queue next serves it. The master
...
We do not measure $D_R$, and we do not measure the portion of it from the switch onward either.
Only the master-facing link is instrumented, so $m_R$ is captured while $t_R$, the response's
arrival at the switch, is not, and the outstation's transmission lies one hop before that. The
quantity
\begin{equation}
\label{eq:resplatency}
m_R - t_R \le D_R
\end{equation}
is therefore a modelled partial journey, written from the model's endpoints rather than from two
timestamps we hold. We use it to say which interval the operational requirement bounds, not as a
measurement, and full $D_R$ remains unmeasured.
...
control plane computes a horizon $H$ of 30.8~ms and an admissible budget of 24.8~ms. Three
different limits act here and we keep them apart: this horizon is advisory and is what the
admission arithmetic reports; a separate clamp in the control plane refuses any acknowledgment
hold above 40~ms outright; and the sweep of Section~\ref{sec:eval:policy} deliberately installs
settings past the horizon, through the same control plane, to find where the mechanism leaves its
operating region. None of the three is a deadline the data plane enforces.
