...
switch forwards each packet onward at some later instant, $e_A$ and $e_R$ respectively. These
are \emph{actual} departures, and we write $\hat{e}_A$ and $\hat{e}_R$ for the instants the
schedule asks for, because the two differ by whatever the switch takes to act on a deadline and
that difference is part of what we report. The
...
\end{equation}
the interval between the two packets arriving at the switch. It is dominated by the time the
outstation spends between acknowledging the request and producing the answer, but it is not a
clean measurement of that time: the two packets traverse the relay-facing link separately, so
differential queueing and the outstation's own transmission both enter it.
...
D_A = e_A - t_A
\end{equation}
for the \emph{acknowledgment hold}. It is written from actual departures, so a configured $D_A$
and the hold a transaction realizes are the same quantity only to the extent that the release
follows its deadline. We refer to $e_R - t_R$ as the \emph{response hold} without giving it a
symbol of its own, because the schedule below fixes it rather than letting us choose it. Substituting into the interval the master observes gives the identity that governs the
whole design,
\begin{equation}
\label{eq:clrt-new}
...
\begin{equation}
\label{eq:schedule}
\hat{e}_A = t_A + D_A , \qquad \hat{e}_R = t_A + D_A + \mathrm{CLRT}_{\mathrm{new}} ,
\end{equation}
where $\mathrm{CLRT}_{\mathrm{new}}$ is the interval we want the master to observe. A packet's
actual departure $e$ follows its scheduled instant $\hat{e}$ by the time the blocker queue takes
to drain and by the queue's own service, which Section~\ref{sec:impl} describes and
Section~\ref{sec:eval:limits} measures on an instrumented build. Writing the two apart is what
keeps the identity honest: (\ref{eq:clrt-new}) holds exactly for actual departures, and
(\ref{eq:schedule}) is a statement about the schedule rather than about the wire. We call this
...
which it leaves when that queue next serves it, as in Figure~\ref{fig:timeline}(b). The master
then usually observes an interval larger than the configured value, though not necessarily: what
it measures is a difference of two releases, each with its own queueing and service, so a late
response is not by itself a guarantee of a larger measured interval. One read-lane exchange in a thousand
...
A third timer is worth naming because it is easy to miss. Holding the response also postpones the
master's acknowledgment of it, which is the feedback the \emph{outstation's} transport waits for,
so the response hold spends the outstation's retransmission budget as well as the application's
patience. That budget is not the master's: on this relay the outstation's timer is about three
orders larger than the master's, so the master's is what binds here, but the three constraints are
separate and a deployment that changed either device could reorder them.
...
The mechanism replaces one interval per lane and leaves the rest of the exchange as it was, within
the scope Section~\ref{sec:threat} sets out. One consequence is worth stating here because the
schedule causes it: the request-to-acknowledgment interval still carries the outstation's own
acknowledgment latency, displaced by the constant $D_A$ but not reshaped by it, and
Section~\ref{sec:eval:ro3} reports what an attacker allowed to retrain recovers from it.
