...
configuration, not a release deadline the hardware guarantees for every held packet, and
Section~\ref{sec:eval:policy} reports the margin we observed.
...
numbers. The two constraints differ in what they bound: the transport bounds $D_A$ because the
...
and the master observed each one, while the delay the mechanism added stayed the same across all
of them. Section~\ref{sec:eval:policy} reports that sweep and the point at which the realized hold
stops tracking the requested one.
