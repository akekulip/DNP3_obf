...
leave the network, so that a timing feature an observer measures becomes a value we choose rather
than a property of the device. A programmable switch at the outstation edge holds the
acknowledgment and the response of a transaction and releases each at a deadline it computes when
the transaction begins.

Because the switch forwards the same packets it received, the endpoints run unmodified firmware
and every DNP3 byte reaches the master unchanged, which is what the plaintext setting requires.
The framework is general over which packets are held and what the released interval should be. We
demonstrate it on two network-observable intervals: the cross-layer response time of a poll,
which is the feature Formby et al.~\cite{formbyWhosControlYour2016} fingerprint, and the
master-visible timing of a select-before-operate control command. The second is our own case and
is not their second feature, which is the physical operation time of the device rather than an
interval read from the network. We evaluate the first on a physical relay. These are two
case studies of one mechanism, not two systems.

\subsection{Contributions}

To overcome the two obstacles above, we design a timing-obfuscation framework that replaces a
selected within-transaction timing feature with a configured value by scheduling the release of
real protocol packets, and we derive the coupling between the acknowledgment hold, the response
hold and the configured interval that any such schedule must satisfy (Section~\ref{sec:design}).

\textbf{Realization.} We realize the framework on an Intel Tofino-1 using paired blocker and hold
queues that sustain a hold until a deadline expires, with a read lane timed from the outstation's
acknowledgment and a control lane timed from the request. Forwarding stays open when a response
arrives after its scheduled release (Section~\ref{sec:impl}).

\textbf{Evaluation.} We evaluate the realization against a physical SEL-751A relay, reporting how
closely the released interval reaches its configured value, what the policy can and cannot cover,
the latency the mechanism adds, the hardware resources it uses, and what an adaptive attacker
