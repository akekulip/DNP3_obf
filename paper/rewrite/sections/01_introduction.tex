...
The framework is general over which packets are held and what the released interval should be, and
we demonstrate it on two network-observable intervals: the cross-layer response time of a poll,
the feature Formby et al.~\cite{formbyWhosControlYour2016} fingerprint, and the master-visible
timing of a select-before-operate control command. The second is our own case rather than their
second feature, which is the device's physical operation time and not an interval read from the
network. These are two case studies of one mechanism, not two systems.
