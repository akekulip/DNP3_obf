...
demonstrate it on the two timing features that Formby et al.~\cite{formbyWhosControlYour2016}
use, the cross-layer response time of a poll and the master-visible timing of a control command,
and we evaluate the first of these over 63{,}360 exchanges on a physical relay. These are two
...
\begin{itemize}
  \item \textbf{Designs a timing-obfuscation framework} that replaces a selected
    ...
    and the target interval that any such schedule must satisfy (Section~\ref{sec:design}).
  ...
  \item \textbf{Evaluates the realization against a physical SEL-751A relay} over 22 collection
    runs, reporting how closely the released interval reaches its target, what the policy can
