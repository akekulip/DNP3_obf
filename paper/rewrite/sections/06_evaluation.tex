...
observation can be informative about the class, whereas the obfuscated ones are a step at the
scheduled release, an instant computed from the acknowledgment and a fixed offset that no longer
depends on how long the device took.
...
sent while earlier bytes were unacknowledged, and there was no retransmission, reset or duplicate
...
and observed the master repeat it after 201~ms, about seven times the longest observed wait, which
is consistent with the absence of a retransmission. The 201~ms is a measured repetition threshold
...
classifier is given both and may exploit any dependence between them, which is what the adaptive
result below does. We estimate mutual information for the CLRT in bits against a null built by
shuffling class labels within each grouped run over 1{,}000 permutations, preserving the per-run
class counts. Every fold leaves out one grouped run, and we report the spread of the 22
...
model, and we do not read it as doing so. Closing the residual requires both paths to be timed
from the same event, which we leave to future work.
...
action and not at the wire, and this program carries no egress timestamp, so the service that
follows is outside the measurement. We report an internal blocker-termination interval and do not
...
endpoints. It comes from a \emph{different build} than the campaign, so it characterizes the
mechanism rather than any exchange in the measured distributions; that build is identified by the
recorded patch over the frozen program, with the build tree, its loader configuration and the
loader's log agreeing on which binary ran. Twelve transactions at one configuration are not a
drain bound over load. Comparing the instrumented build against the frozen one over 200 polls
each, extracted the same way, gives mean cross-layer response times of 4.000282 and 3.999470~ms
and sample standard deviations of 0.0153 ms in each: one pair of captures at one configuration,
not a demonstration that the instrumentation is free. It does not establish equivalence, and
because the measured interval is a difference of two releases it would not reveal equal movement
in both.
...
reached the master's interface, at 3.20, 9.21 and 20.25~s, every one forwarded rather than
absorbed.

What that does \emph{not} show is that duplicate suppression preserves loss recovery, for two
reasons the distinction turns on. The filter dropped every copy for the whole test rather than
only the original, so nothing was acknowledged and the application recovered nothing, and the
capture is taken ahead of the filter, so it records arrival at the interface. The copies also
arrived long after the transaction had retired, the first 2.97~s after a transaction live for
about 24~ms, so they met the path taken by a stale token rather than by a duplicate of a live
transaction, which is the case the concern is about. Reaching that path needs an outstation whose
retransmission timer is of the same order as the hold; on this relay it is about three orders
larger. We report the mechanism as forwarding late copies, and leave the live-window case open
rather than closed.
