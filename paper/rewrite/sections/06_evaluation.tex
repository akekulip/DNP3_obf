...
\begin{figure}[tbp]
  ...
  \caption{Transaction-class leakage under the two attacker models, leaving out one grouped run at
  a time. (a) Balanced accuracy by attacker condition and feature set; whiskers span the 22
  held-out runs and are not a confidence interval. (b) Mutual information between the CLRT and the
  class, against a within-run permutation null.}
  \label{fig:leakage}
\end{figure}
...
both and may exploit any dependence, which is what the adaptive result below does. Mutual
information for the CLRT is estimated in bits against a null built by shuffling class labels
within each grouped run over 1{,}000 permutations, which preserves the per-run class counts.
...
from different events, so their acknowledgment latencies differ. Row-normalised confusion over all
22 held-out runs, published with the artifact, shows the shape of that recovery: OPERATE separable
while SELECT stays confused with READ.
