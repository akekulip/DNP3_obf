...
device's timing and releases each at a deadline computed when the transaction begins, from the
outstation's acknowledgment on the read path, which is READ and the SELECT phase of
select-before-operate, and from the request for OPERATE. Consequently, the interval an observer
sees is the operator's value rather than the device's.
