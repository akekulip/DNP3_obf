...
Defenses in this setting have so far worked at the content and connectivity layer: DefRec virtualizes physical
...
setting assumes an encrypted payload. Our setting differs in both respects rather than in the
soundness of those methods: it carries a plaintext protocol whose leaking feature is one interval
inside a request-response exchange, and no endpoint can be modified to help. Because the outstation cannot be changed, it adds no
