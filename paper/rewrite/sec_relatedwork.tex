...
device types from wire-side timing signatures. This line establishes the threat the defense addresses:
...
traffic. The defense differs in deployment and constraint; it normalizes the ACK-to-response timing of a
...
lines change or pad bytes and target the end host. The defense differs in mechanism; it neither pads nor
...
layer. The defense differs in observation point; it obfuscates the wire-visible size and timing of the
...
and CRCs the receiver validates. The defense differs in two ways: it is byte-preserving and carves only
at existing DNP3 boundaries, so the outstation's protocol remains valid, and it adds a control-command
mode that hides operation timing without exposing the hold to a passive observer.
