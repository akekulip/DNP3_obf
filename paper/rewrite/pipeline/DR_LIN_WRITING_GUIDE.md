...
* Every number traces to `timing_stats.json` or a derived CSV regenerated by `reproduce.sh`.
* Every figure sentence names its figure and reports the observation point.
* The Obfuscated mutual information is quoted as "below 0.003 bits and inside the permutation
  null", not to four significant figures.
* The limitations in `CLAIMS_AND_LIMITATIONS.md` L1 to L10 appear in the Evaluation's
