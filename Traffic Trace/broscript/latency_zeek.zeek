...
# PORTED: open_log_file("latency") was removed in modern Zeek; use open() instead.
global latency_log = open("latency.log");
...
event dnp3_application_response_header(c: connection, is_orig: bool, application_control: count, fc: count, iin: count) {
    ...
    if (c$id$resp_h !in req_time) {
        ...
        if (cur_seq !in req_time[c$id$resp_h]) {
            ...
            # PORTED/FIXED: original created the per-function-code vector only
            # inside the per-IP init block, so a response with a new fc crashed
            # with "no such index". Init the fc vector on every new fc instead.
...
# PORTED: bro_done() was renamed to zeek_done() in modern Zeek.
event zeek_done() {
