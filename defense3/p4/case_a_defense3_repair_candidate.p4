...
/* ---- queues on PORT_L (CONSENSUS §1: exactly two) ---- */
const bit<5> QID_BLOCK = 5w7;   /* Q_BLOCK: HIGH (max_priority=7) : blocker reservoir  */
/* D3: was QID_RESP. Same qid, new occupant: the ACK first, then the RESPONSE. */
const bit<5> QID_HOLD  = 5w1;   /* Q_HOLD : LOW  (max_priority=0) : held ACK + RESPONSE */
...
control Ingress(inout headers_t hdr,
    ...
    RegisterAction<bit<8>, bit<1>, bit<8>>(reg_tag) tag_arm = {
        void apply(inout bit<8> v, out bit<8> rv) {
            rv = meta.gen_in - v;
            if (v == TAG_INACTIVE) { v = meta.gen_in; }
        }
    };
#endif
...
#ifndef D3_REPAIR_R1
                    /* E1 (ORIGINAL, DEFECTIVE): the marker delta, set HERE because
                     * tag_read_or_mark runs at level 2 and the decode table that would be
                     * the natural home for it runs at level 3. ►► THE DEFECT: this is
                     * conditioned on direction, session and DNP3 framing ONLY. A response
                     * with a WRONG TCP SEQUENCE still reaches the SALU and still marks,
                     * because the SALU's own guard tests the STORED state, not this
                     * packet's validity. */
