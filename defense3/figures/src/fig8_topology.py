...
def node(x, y, w, h, fc, t1, t2):
    ax.add_patch(FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.08", fc=fc,
...
node(3.55, 3.55, 3.30, 2.45, C[0], "Tofino-1 switch", "Defense 3 runs here\n(2 pipes, 32 ports)")
node(7.75, 4.05, 2.55, 1.45, "0.93", "SEL-751 relay", "the outstation\n192.168.10.7")

def link(x1, x2, y, lab, sub, col="black"):
    ax.add_patch(FancyArrowPatch((x1, y), (x2, y), arrowstyle="<|-|>", mutation_scale=6,
                 lw=1.1, color=col, zorder=2, shrinkA=0, shrinkB=0))
    ax.text((x1+x2)/2, y + 0.18, lab, fontsize=6.1, ha="center", va="bottom")
link(2.65, 3.55, 4.78, "port 9", "")
link(6.85, 7.75, 4.78, "port 64", "")

# the internal loopback that does the actual holding
ax.add_patch(Rectangle((3.55, 2.05), 3.30, 1.15, fc="0.965", ec="0.6", lw=0.7, zorder=3))
...
ax.add_patch(FancyArrowPatch((4.55, 3.55), (4.55, 3.20), arrowstyle="-|>",
             mutation_scale=6, lw=0.9, color=C[3], zorder=4))
...
ax.add_patch(FancyArrowPatch((3.40, 1.40), (3.14, 4.58), arrowstyle="-|>",
             mutation_scale=6, lw=0.9, color=C[2], ls="--", zorder=2))
...
ax.text(0.10, 6.60, "The relay is untouched. No packet byte is modified. Only the "
        "TIMING\nof the ACK leaving port 9 is changed. Port 9 runs at 25 Gb/s, port 64 at 1 Gb/s.", fontsize=6.3, ha="left",
