...
ax.bar(x, share_tmo, 0.62, color=TMOC, ec="black", lw=0.6,
       label="ends as intended — budget runs out (TMO): exactly 1")
ax.bar(x, share_stl, 0.62, bottom=share_tmo, color=STLC, ec="black", lw=0.6,
       hatch=STL_HATCH,
       label="ends stale — the tag was already erased: the other $K{-}1$")
...
ax.set_xlabel("reservoir size $K$ (number of tokens; deployed $K$ = 64)",
              fontweight="bold")
ax.set_ylabel("share of the $K$ tokens (%)", fontweight="bold")
ax.set_title("(a) unrepaired: only 1 of the $K$ tokens ends as intended — at every $K$",
             fontsize=8.4, pad=3)
...
ax.annotate("only 1 ends as\nintended (TMO)", xy=(0.25, 1), xytext=(0.50, 10),
            fontsize=6.4, color="0.2",
            arrowprops=dict(arrowstyle="-|>", lw=0.7, color="0.2"))
ax.set_xticks(x); ax.set_xticklabels(groups)
ax.set_ylim(0, 70); ax.set_yticks([0, 16, 32, 48, 64])
ax.set_ylabel("tokens ($K=64$)", fontweight="bold")
ax.set_title("(b) repair R2 keeps the tag: all 64 end as intended", fontsize=8.4, pad=3)
utils_mpl.set_grid(fig, ax)
ax.xaxis.grid(False)
